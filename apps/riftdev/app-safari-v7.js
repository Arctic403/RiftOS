...
async function importRepoFromGitHub(token, repo, branch) {
    ...
    try {
        ...
        if (!filesToFetch.length) {
            ...
            loadFiles();
            setGitSyncProgress("Pull complete. Branch contains no files.", "success");
        ...
        for (const file of filesToFetch) {
            ...
            setGitSyncProgress(`Downloading ${imported + 1}/${filesToFetch.length}: ${file.path}`);
            try {
                ...
                baselineFiles[file.path] = {
                    ...
                    localHash: await getWorkspaceContentHash(file.path, content),
        ...
        loadFiles();
        if (failed.length) {
            setGitSyncProgress(`Pull incomplete: ${failed.length} file(s) failed. Sync baseline was not replaced.`, "error");
            const preview = failed.slice(0, 8).join("\n");
            alert(`Pulled ${imported} of ${filesToFetch.length} file(s) from ${repo} (${branch}).\n\nSync baseline was NOT updated because some files failed.\n\nFailed:\n${preview}${failed.length > 8 ? `\n...and ${failed.length - 8} more` : ""}`);
        } else {
            saveGitSyncState({
                version: 2,
                repo,
            ...
            setGitSyncProgress(`Pull complete: ${imported} files synced @ ${commitSha.slice(0, 7)}.`, "success");
