...
(() => {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .ide-v9-toolbar-tools.hidden {
                display:none;
            }

            .ide-v9-toolbar-tools button {
                font-size:12px;
            }
            ...
            @media (max-width:650px) {
                ...
                .editor-actions.ide-mobile-collapsible {
                ...
                .editor-actions.ide-mobile-collapsible.ide-open {
                    display:flex !important;
                }

                .editor-actions.ide-mobile-collapsible > button {
                    flex:none;
                }
    ...
    function injectToolbar() {
        ...
        toggle.innerHTML = '<span>🧰 IDE Tools</span><span id="ideToolsChevron">▾</span>';

        const tools = document.createElement("div");
        tools.id = "ideV9ToolbarTools";
        tools.className = "ide-v9-toolbar-tools hidden";
        tools.append(
            createButton("🔎 Project Search", showProjectSearch),
            createButton("🧾 Git Diff", showGitDiff),
            createButton("🩺 Diagnostics", runDiagnostics),
            createButton("▶ Preview", previewProject),
            createButton("🎯 Definition", goToDefinition),
            createButton("🔗 References", findReferences),
            createButton("📥 Auto Import", autoImportSelected),
            createButton("✏ Rename File", renameCurrentFile),
            createButton("🕘 History", showHistory),
            createButton("💾 Save Data", showSaveDataViewer),
            createButton("🛠 RiftCity Dev", showRiftCityDevPanel)
        );

        toggle.addEventListener("click", () => {
            const hidden = tools.classList.toggle("hidden");
            const chev = $("ideToolsChevron");
        ...
        if (breadcrumb) {
            editorSection.insertBefore(bar, breadcrumb);
        } else {
            editorSection.prepend(bar);
        }
    }
    ...
    function clearClosedEditor() {
        ...
        if (typeof updateHighlight === "function") {
            try { updateHighlight(); } catch {}
        }

        if (typeof updateLineNumbers === "function") {
            try { updateLineNumbers(); } catch {}
        }
    }
    ...
    async function switchToTab(path) {
        ...
        // Do not mark the tab active until the core editor has actually
        // finished opening the file. This avoids Safari/capture-event races.
        await openFile(path);
    ...
    function renderTabs() {
        ...
        if (!openTabs.length) {
            ...
            blank.className = "ide-small";
            blank.textContent = "No open files";
        ...
        openTabs.forEach(path => {
            ...
            // pointerup makes the close target reliable on iOS Safari even
            // when the horizontal tab row has just been scrolled.
    ...
    function setupMobileEditorActions() {
        ...
        if (!header || !actions || $("ideEditorActionsToggle")) return;

        actions.classList.add("ide-mobile-collapsible");

        const toggle = document.createElement("button");
        toggle.type = "button";
        toggle.id = "ideEditorActionsToggle";
        toggle.className = "ide-editor-actions-toggle";
        toggle.innerHTML = '<span>⌨️ Editor Actions</span><span id="ideEditorActionsChevron">▾</span>';

        toggle.addEventListener("click", () => {
            const open = actions.classList.toggle("ide-open");
            const chev = $("ideEditorActionsChevron");
    ...
    function hookCoreCloseButton() {
        ...
        // Capture the click before the v8 handler clears data-filename.
        // The tab manager then owns the entire close/switch operation.
        button.addEventListener("click", async event => {
            const editor = $("editor");
            const path = editor?.dataset.filename || activeTab || "";
            if (!path) return;
    ...
    function watchActiveFile() {
        ...
        // The core editor changes data-filename whenever openFile() completes.
        // Watching that attribute is enough to track files opened from the
        // workspace tree. The old document-wide capture click listener raced
        // tab clicks and re-selected/re-opened the previous file.
