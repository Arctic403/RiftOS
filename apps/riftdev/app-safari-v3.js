...
function initDatabase() {
    ...
    dbReadyPromise = new Promise((resolve, reject) => {
        ...
        const fail = (error) => {
            dbReadyPromise = null;
            reject(error || new Error("IndexedDB failed to open."));
        };

        const finishOpen = (openedDb) => {
            db = openedDb;
            settled = true;
            ...
            // Keep these after the connection is usable. They are intentionally
            // non-blocking so callers waiting on initDatabase() can proceed.
            Promise.resolve().then(() => loadFiles()).catch(console.error);
            restoreSettings();
            resolve(db);
        };

        const openUpgrade = (nextVersion) => {
            const upgradeRequest = window.indexedDB.open(DB_NAME, nextVersion);
            ...
            upgradeRequest.onsuccess = function (event) {
                finishOpen(event.target.result);
            };

            upgradeRequest.onerror = function () {
                fail(upgradeRequest.error || new Error("IndexedDB schema upgrade failed."));
            };

            upgradeRequest.onblocked = function () {
                console.warn("IndexedDB schema upgrade is blocked by another tab/session.");
            };
        };

        // IMPORTANT: Do not pass a hard-coded version here. Safari may already
        // have a newer LocalWorkspaceDB from an older deployment. Opening the
        // database without a version always uses the version that actually
        // exists, avoiding VersionError / attempted database downgrade failures.
        const request = window.indexedDB.open(DB_NAME);

        // This fires only when the DB does not exist yet (created at version 1).
        ...
        request.onsuccess = function (event) {
            ...
            // Defensive migration: if an existing database somehow lacks the
            // files store, advance from its ACTUAL version rather than assuming
            // a fixed number.
            const nextVersion = openedDb.version + 1;
            openedDb.close();
            openUpgrade(nextVersion);
        };

        request.onerror = function () {
            fail(request.error || new Error("IndexedDB failed to open."));
        };
