...
class RiftAiJournal(context: Context) {
    ...
    fun beginSession(task: String): JSONObject {
        ...
        pruneOldSessions(id)
        recordEvent("session", "AI session started", JSONObject().put("id", id))
        return state()
    }
    ...
    fun state(): JSONObject {
        ...
        val meta = readObject(File(sessionDir(id), "meta.json")) ?: JSONObject().put("id", id)
        return JSONObject()
            .put("active", true)
            .put("localOnly", true)
            .put("transport", "ChatGPT Web")
    ...
    @Synchronized
    fun captureForTool(name: String, args: JSONObject) {
        if (activeSessionId() == null) return
        when (name) {
            "rift_write_text", "rift_mkdir", "rift_remove" -> capturePath(args.optString("path"))
            "rift_move" -> {
                capturePath(args.optString("from"))
                capturePath(args.optString("to"))
            }
        }
    }

    @Synchronized
    fun recordTool(name: String, args: JSONObject, phase: String, ok: Boolean? = null, error: String? = null) {
        if (activeSessionId() == null) return
        val data = JSONObject()
            .put("tool", name)
    ...
    fun changes(): JSONArray {
        ...
        for (index in 0 until snapshots.length()) {
            val snapshot = snapshots.optJSONObject(index) ?: continue
            val change = changeForSnapshot(id, snapshot) ?: continue
            out.put(change)
        }
        return out
    }
    ...
    fun acceptAll(): JSONObject {
        ...
        recordEvent("review", "Accepted $count change${if (count == 1) "" else "s"}", JSONObject().put("changes", count))
    ...
    fun revertAll(): JSONObject {
        ...
        File(sessionDir(id), "originals").mkdirs()
        writeJson(File(sessionDir(id), "snapshots.json"), JSONArray())
        recordEvent("review", "Reverted $count change${if (count == 1) "" else "s"}", JSONObject().put("changes", count))
        return JSONObject().put("reverted", true).put("changes", count)
    }

    private fun capturePath(rawPath: String) {
        val id = activeSessionId() ?: return
        val path = normalizePath(rawPath)
        require(path.isNotBlank()) { "Rift AI cannot snapshot the sandbox root" }
        if (findSnapshot(id, path) != null) return
    ...
    private fun restoreSnapshot(id: String, snapshot: JSONObject) {
        ...
        when (snapshot.optString("kind")) {
            ...
            "directory" -> {
                target.parentFile?.mkdirs()
                originalFile(id, snapshot).copyRecursively(target, overwrite = true)
            }
        }
    }
