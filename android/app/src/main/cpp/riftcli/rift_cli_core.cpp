...
std::string baseStatus(const std::string& command, const std::string& cwd) {
    ...
    out
        ...
        << "\"coreVersion\":\"0.1.1-driver-live-poll\","
        ...
        << "\"driverToolExecution\":\"live-poll-jobs\","
...
CommandResponse execute(const std::vector<std::string>& args, const std::string& cwd) {
    ...
    if (command == "help") {
        const std::string output =
            ...
            "CLI actions run as live-poll jobs: recover with rift_cli_job_list, poll with rift_cli_job_poll, or cancel with rift_cli_job_cancel. "
