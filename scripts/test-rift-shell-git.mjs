...
const canonical=value=>{const path=normalize(value);if(path==='/D:/Workspace')return '/workspace';if(path.startsWith('/D:/Workspace/'))return '/workspace/'+path.slice('/D:/Workspace/'.length);return path;};
const context={window:{RiftOSCore:{ready:Promise.resolve(),fs,path:{isAbsolute,canonical}}},fetch,sessionStorage,crypto:webcrypto,TextEncoder,Uint8Array,atob:value=>Buffer.from(value,'base64').toString('binary'),console,prompt:()=>''};
...
output.length=0;await context.window.RiftGit.run(['-C','D:/Workspace/RiftOS-main','status'],print,{cwd:'/home'});assert(output.some(line=>line.includes('root /workspace/RiftOS-main')));
output.length=0;
...
assert.equal(Buffer.from(files.get('/home/.riftgit-current'),'base64').toString(),'/home/RiftOS-main');
console.log('ok - bare D:/Workspace git -C canonicalizes to the existing checkout');
