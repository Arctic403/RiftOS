...
assert.ok(runner.includes('scanTrainingMetadata(trainFile)'));
assert.ok(runner.includes('DigestInputStream'));
assert.equal((runner.match(/loadTraining\(trainFile\)/g)||[]).length,1,'status must not load training token arrays');
