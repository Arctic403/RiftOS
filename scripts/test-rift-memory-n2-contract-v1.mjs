...
n2Assert.equal(n2PhaseAuthority.programStatus, 'N2.0-N2.6 PROMOTED / N2-M1 + N2-M2 + N2-M3 PROMOTED; N2.7-N2.8 SOURCE IMPLEMENTED / N2-M4 SOURCE IMPLEMENTED; N2.9-N2.12 PENDING');
n2Assert.equal(n2PhaseAuthority.runtimeStatus, 'N2 CANONICAL MEMORY RUNTIME INACTIVE; N2-M1 + N2-M2 + N2-M3 PROMOTED DIAGNOSTICS; N2-M4 SOURCE-IMPLEMENTED DIAGNOSTIC ONLY');
...
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'promoted').length, 7);
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'source-implemented').length, 2);
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'pending').length, 4);
...
for (const row of n2PhaseAuthority.phases.slice(9)) {
...
n2Assert.equal(n2PhaseAuthority.macroImplementationPlan[3].status, 'source-implemented');
n2Assert.ok(n2PhaseAuthority.macroImplementationPlan.slice(4).every(row => row.status === 'pending'));
