...
n2Assert.equal(n2PhaseAuthority.programStatus, 'N2.0-N2.4 PROMOTED / N2-M1 + N2-M2 PROMOTED; N2.5 + N2.6 SOURCE-IMPLEMENTED / N2-M3 PROMOTION PENDING; N2.7-N2.12 PENDING');
n2Assert.equal(n2PhaseAuthority.runtimeStatus, 'N2 CANONICAL MEMORY RUNTIME INACTIVE; N2-M3 DIAGNOSTIC ONLY');
...
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'promoted').length, 5);
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'source-implemented').length, 2);
n2Assert.equal(n2PhaseAuthority.phases.filter(row => row.status === 'pending').length, 6);
...
for (const row of n2PhaseAuthority.phases.slice(7)) {
...
n2Assert.equal(n2PhaseAuthority.macroImplementationPlan[2].status, 'source-implemented/promotion-pending');
n2Assert.ok(n2PhaseAuthority.macroImplementationPlan.slice(3).every(row => row.status === 'pending'));
