...
const chatHandoff = readFileSync('android/app/src/main/java/com/riftos/app/RiftChatHandoff.kt', 'utf8');
const shellBridge = readFileSync('android/app/src/main/java/com/riftos/app/RiftShellBridge.kt', 'utf8');
...
const checks = [
  ...
  ['maximize/restore preserves exact pre-maximize geometry without late async overwrite', desktop.includes('riftRestoreGeometry') && desktop.includes('currentGeometry(win)') && desktop.includes('bumpGeometryRevision(win)') && desktop.includes("String(win.dataset.riftGeometryRevision||'0')!==revision") && desktop.includes("windowActionLabel(win,'Restore')") && desktop.includes("windowActionLabel(win,'Maximize')")],
  ['core icon-only shell controls expose semantic accessibility names', filesUi.includes('id="fsBack" title="Back" aria-label="Back"') && filesUi.includes('id="fsRefresh" title="Refresh" aria-label="Refresh"') && filesUi.includes('aria-label="Browser Back"') && filesUi.includes('aria-label="Browser Reload"') && filesUi.includes('aria-label="Browser address"') && filesUi.includes('id="editorText" aria-label="Editor text"') && desktop.includes("windowActionLabel(win,'Minimize')") && desktop.includes("close.setAttribute('aria-label',close.title)") && desktop.includes("button.setAttribute('aria-label',meta.label)")],
  ['taskbar only retains pinned or running apps and keeps semantic app names', desktop.includes("btn.classList.toggle('rift-taskbar-hidden',!pinned&&!item)") && desktop.includes('taskbarPins') && desktop.includes('pinTaskbar(id,pinned=true)') && desktop.includes("btn.setAttribute('aria-label',meta.label)") && desktop.includes("button.setAttribute('aria-label',meta.label)")],
  ...
  ['Vortex foreground sessions are one-call, native, bounded, and keep the MCP tool family unchanged', filesUi.includes('core.native.call("vortex.session"') && filesUi.includes('test-wait') && filesUi.includes('script-wait') && dispatcher.includes('"vortex.session" -> vortexBridge.executeSession(args)') && vortexBridge.includes('fun executeSession(rawArgs: JSONObject)') && vortexBridge.includes('RiftVortexLocalAgent.ensureActiveForSession(appContext)') && vortexBridge.includes('SESSION_TIMEOUT_MS = 85_000L') && core.includes('if(method==="vortex.session")return 95*1000') && !host.includes('rift_vortex_session')],
  ...
  ['Fixed-scope local agents are Android-package-scoped and user-granted', androidManifest.includes('RiftVortexAccessibilityService') && androidManifest.includes('android.permission.BIND_ACCESSIBILITY_SERVICE') && vortexAgentConfig.includes('android:packageNames="com.vortex3d.app,com.riftos.app"') && vortexAgentConfig.includes('android:canPerformGestures="true"') && vortexAgent.includes('TARGET_PACKAGE = "com.vortex3d.app"') && vortexAgent.includes('TARGET_PACKAGE = "com.riftos.app"') && vortexAgent.includes('last foreground package was')],
  ['Fixed-scope local agents self-activate and stabilize only constructor-fixed targets inside one native action', vortexAgent.includes('ACTIVATION_TIMEOUT_MS = 3_000L') && vortexAgent.includes('ROOT_SETTLE_MS = 120L') && vortexAgent.includes('REQUIRED_STABLE_ROOT_POLLS = 2') && vortexAgent.includes('context.startActivity(launch)') && vortexAgent.includes('root.window?.isActive') && vortexAgent.includes('stablePolls >= REQUIRED_STABLE_ROOT_POLLS') && vortexAgent.includes('actions_auto_activate') && vortexAgent.includes('internal fun ensureActiveForSession(context: Context)')],
  ['Local agents cannot become arbitrary app/root/ADB control', dispatcher.includes('"vortex.agent" -> RiftVortexLocalAgent.execute') && dispatcher.includes('"riftos.agent" -> RiftOsLocalAgent.execute') && filesUi.includes('cmd==="vortex-agent"') && filesUi.includes('cmd==="riftos-agent"') && shellBatch.includes('"vortex-agent"') && shellBatch.includes('"riftos-agent"') && vortexAgent.includes('getLaunchIntentForPackage(targetPackage)') && vortexAgent.includes('Password fields are not available') && !vortexAgent.includes('optString("package"') && !vortexAgent.includes('Runtime.getRuntime') && !vortexAgent.includes('ProcessBuilder') && !vortexAgent.includes('"adb"')],
  ['Local-agent semantic actions reject stale hidden and ambiguous targets and settle after input', vortexAgent.includes('node.isVisibleToUser && node.isEnabled') && vortexAgent.includes('exactMatches') && vortexAgent.includes('semanticMatches') && vortexAgent.includes('UI target is ambiguous') && vortexAgent.includes('semanticLabel') && vortexAgent.includes('ACTION_SETTLE_MS = 140L') && vortexAgent.includes('settled_ms')],
  ['RiftShell open routes dynamic system apps explicitly and rejects missing apps', filesUi.includes('app==="mcp"') && filesUi.includes('RiftMcp?.open') && filesUi.includes('app==="riftrt"') && filesUi.includes('RiftRT?.openManager') && filesUi.includes('app not found: ${app}')],
  ['Files destination picker stays shallow, same-folder aware and reject-safe', filesUi.includes('folderOnly?core.path.normalize(startPath):core.path.parent(startPath)') && filesUi.includes('core.fs.list(current,{recursive:false})') && filesUi.includes('return new Promise((resolve,reject)=>') && filesUi.includes('render().catch(fail)') && filesUi.includes('if(core.path.parent(entry.path)===destinationDir)continue')],
  ['Files context menu shares real Open/Paste paths and long-press cancels on movement', filesUi.includes("if(action==='open')await openEntry(entry)") && filesUi.includes("if(action==='paste')") && filesUi.includes('pasteInto(destination)') && filesUi.includes("action==='zip'&&!protectedFileRoots.has(entry.path)") && filesUi.includes('Math.hypot(event.clientX-pressStart.x,event.clientY-pressStart.y)>8') && filesUi.includes('button.onpointercancel=cancelLongPress')],
  ['Built-in window processes terminate through one idempotent lifecycle', filesUi.includes('function closeWindow(target,{fromProcess=false}={})') && filesUi.includes('if(record.closing)return true') && filesUi.includes('if(!fromProcess)') && filesUi.includes('onTerminate:()=>closeWindow(win,{fromProcess:true})') && filesUi.includes('closing:false')],
  ['Files selection cancellation and Editor save state survive async UI races', !filesUi.includes('const destinationPicker=') && filesUi.includes("window.addEventListener('pointercancel',finish,{once:true})") && filesUi.includes('let editRevision=0') && filesUi.includes('if(editRevision===revision)') && filesUi.includes('Editor · save failed')],
  ['Rift app managers surface async failures and confirm destructive removal', riftApps.includes('Its saved app data will also be deleted.') && riftApps.includes('Demo install failed:') && riftApps.includes('Remove failed:') && riftRt.includes('Its saved app data will also be deleted.') && riftRt.includes('Demo install failed:') && riftRt.includes('Remove failed:')],
  ['Virtual mouse window motion obeys maximize and geometry revision guards', desktop.includes("win&&!win.classList.contains('rift-maximized')&&target.closest('.window-bar')") && desktop.includes("win&&!win.classList.contains('rift-maximized')&&target.closest('.rift-window-resizer')") && desktop.includes('bumpGeometryRevision(win);const rect=win.getBoundingClientRect()') && desktop.includes("card.addEventListener('pointercancel',end,{once:true})")],
  ['RiftRT close hands focus to the next shared desktop window and show-desktop emits once', riftRt.includes("stage.querySelectorAll('.window.rift-desktop-window:not(.rift-minimized)')") && riftRt.includes('RiftOSWindowManager?.focus?.(next.dataset.app||next)') && !riftRt.includes("globalThis.dispatchEvent(new Event('riftos:show-desktop'))")],
  ['Chat handoff stays behind existing RiftShell authority without a new MCP tool family', filesUi.includes('cmd==="chat"') && filesUi.includes('core.native.call("chat.handoff"') && dispatcher.includes('"chat.handoff" -> RiftChatHandoff.execute') && shellBatch.includes('"chat"') && gradle.includes('RiftChatHandoff.kt') && core.includes('chatHandoff:true') && !host.includes('rift_chat')],
  ['Chat handoff is app-private RiftFS-only, bounded, hashed, and non-networked', chatHandoff.includes('BUNDLE_SCHEMA = "rift.chat-handoff/1"') && chatHandoff.includes('BUNDLE_DIR = "workspace/chat-handoffs"') && chatHandoff.includes('requireInside(riftRoot, file)') && chatHandoff.includes('allowedEntries') && chatHandoff.includes('MAX_TRANSCRIPT_BYTES') && chatHandoff.includes('MessageDigest.getInstance("SHA-256")') && !chatHandoff.includes('http://') && !chatHandoff.includes('https://') && !chatHandoff.includes('Socket(')],
  ...
  ['RiftShell long Vortex sessions get a dedicated bounded envelope without widening normal shell calls', shellBridge.includes('VORTEX_SESSION_TIMEOUT_MS = 105_000L') && shellBridge.includes('test-wait|validate-wait|script-wait')],
  ...
  ['C++ include resolution is path-only and never guesses by symbol name', sandbox.includes('val suffixMatches = allPaths.filter { it.endsWith("/$specifier") }') && sandbox.includes('val includeMatches = suffixMatches.filter { it.contains("/include/") }') && sandbox.indexOf('return null', sandbox.indexOf('if (dependency.kind == "include")')) < sandbox.indexOf('if (dependency.kind == "python")')],
  ['validation test discovery uses normalized path and token affinity', sandbox.includes('validationQueryTokens(projectRelativeQuery)') && sandbox.includes('validationTestAffinity(testPath, projectRelativeQuery, queryTokens)') && sandbox.includes('camelSplit')],
  ...
  ['Workspace Records mounts in the trusted shell without an iframe', workspaceHost.includes('attachShadow({mode:"open"})') && workspaceHost.includes('mountWorkspaceRecords(shadow,invoke,subscribe)') && !workspaceHost.includes('<iframe') && !workspaceHost.includes('postMessage')],
  ['Workspace Records uses direct records calls and cleans up watcher subscriptions', workspacePageScript.includes('invoke(method,args)') && workspacePageScript.includes('subscribe(handleNativeEvent)') && workspacePageScript.includes('unsubscribe();') && !workspacePageScript.includes('parent.postMessage') && !workspacePage.includes('src="./app.js"')],
