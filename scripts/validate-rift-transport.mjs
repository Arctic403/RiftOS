...
const checks = [
  ...
  ['MainActivity is renderer-free', !main.includes('android.webkit') && !main.includes('androidx.webkit') && !main.includes('WebView') && !main.includes('addJavascriptInterface')],
  ['Rift++ uses bounded headless QuickJS', shell.includes('headlessJs.executeRiftpp(args, cwd)') && headless.includes('quickJs {') && headless.includes('evaluationTimeoutMillis') && !headless.includes('WebView') && !headless.includes('Socket(') && !headless.includes('ProcessBuilder')],
  ['RiftBrowser owns file chooser and WebView app surfaces', browserWindow.includes('WebChromeClient.FileChooserParams') && browserWindow.includes('FILE_CHOOSER_REQUEST') && browserHost.includes('WebViewCompat.addWebMessageListener') && browserHost.includes('app.riftos.local')],
  ...
  ['Dev Lab is native and browser execution is explicitly separated', devLab.includes('object RiftNativeDevLab') && services.includes('Web execution/HTML preview is owned by RiftBrowser') && !workspaceApps.includes('android.webkit')],
  ['Workspace Records is local record-only native UI', workspaceApps.includes('WORKSPACE RECORDS · LOCAL ONLY') && workspaceRecords.includes('riftfs/workspace') && !workspaceApps.includes('reconnect') && !workspaceApps.includes('approve')],
  ...
  ['MCP retry dedupe is relay-scoped', server.includes('retryKey') && server.includes('private val completed') && relayClient.includes('server.handleAsync(payload, requestId)')],
  ...
  ['relay ignores stale socket events and does not persist payloads', (relayWorker.match(/if \(socket !== this\.socket\) return;/g) || []).length >= 3 && relayWorker.includes('MAX_PENDING_REQUESTS = 128') && relayWorker.includes('readBoundedText') && relayWorker.includes('encoder.encode(raw).byteLength') && relayWorker.includes('Malformed RiftOS MCP response') && relayWorker.includes('mcp.notification') && relayClient.includes('mcp.notification') && !relayWorker.includes('ctx.storage')],
