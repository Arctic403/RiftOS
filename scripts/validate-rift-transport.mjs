...
const checks = [
  ...
  ['RiftCLI push channel is bounded, replayable and hibernatable', cliEvents.includes('SCHEMA = "rift.cli-event/1"') && cliEvents.includes('MAX_EVENTS = 256') && cliEvents.includes('MAX_EVENT_BYTES = 96 * 1024') && cliEvents.includes('AtomicLong(System.currentTimeMillis() * 1000L)') && cliEvents.includes('type.trim().take(128)') && cliEvents.includes('val sequenceValue = event.optLong("sequence")') && cliEvents.includes('fun replayAfter(afterSequence: Long)') && runtime.includes('fun cliEvents(): RiftCliEventBus') && relayClient.includes('cliEvents.addListener(cliEventListener)') && relayClient.includes('"cli.replay.request"') && relayClient.includes('.put("cliAckSequence", lastCliAckSequence)') && relayWorker.includes('ctx.getWebSockets("driver")') && relayWorker.includes('acceptWebSocket(server, ["driver"])') && relayWorker.includes('request.headers.get("last-event-id")') && relayWorker.includes('minimumCliResumeAfter()') && relayWorker.includes('deviceAttachment.lastCliSequence') && relayWorker.includes('lastCliSequence: this.lastCliSequence') && relayWorker.includes('message.cliAckSequence ?? 0') && relayWorker.includes('deviceAckSequence') && relayWorker.includes('deliveredThrough >= sequence') && relayWorker.includes('after >= sequence') && relayWorker.includes('existing.length >= MAX_DRIVER_SOCKETS') && relayWorker.includes('effectiveClientCount >= MAX_SSE_CLIENTS') && relayWorker.includes('client.controller.desiredSize < sse.byteLength') && relayWorker.includes('notifications/riftcli/event') && !relayWorker.includes('ctx.storage')],
