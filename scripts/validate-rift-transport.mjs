...
const checks = [
  ...
  ['Vortex local agent is Android-package-scoped and user-granted', androidManifest.includes('RiftVortexAccessibilityService') && androidManifest.includes('android.permission.BIND_ACCESSIBILITY_SERVICE') && vortexAgentConfig.includes('android:packageNames="com.vortex3d.app"') && vortexAgentConfig.includes('android:canPerformGestures="true"') && vortexAgent.includes('TARGET_PACKAGE = "com.vortex3d.app"') && vortexAgent.includes('last foreground package was')],
  ['Vortex local agent self-activates only its hard-coded target inside one native action', vortexAgent.includes('ACTIVATION_TIMEOUT_MS = 3_000L') && vortexAgent.includes('context.startActivity(launch)') && vortexAgent.includes('if (lastPackage == TARGET_PACKAGE && root != null) return service to root') && vortexAgent.includes('actions_auto_activate') && vortexAgent.includes('internal fun ensureActiveForSession(context: Context)')],
