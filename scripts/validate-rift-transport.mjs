...
const checks = [
  ...
  ['MainActivity is renderer-free', !/(?:^|\n)\s*import\s+(?:android|androidx)\.webkit\b|(?:android|androidx)\.webkit\./m.test(main) && !main.includes('addJavascriptInterface')],
  ['Rift++ uses bounded headless QuickJS', shell.includes('headlessJs.executeRiftpp(args, cwd)') && headless.includes('quickJs {') && headless.includes('evaluationTimeoutMillis') && !/(?:^|\n)\s*import\s+(?:android|androidx)\.webkit\b|(?:android|androidx)\.webkit\./m.test(headless) && !headless.includes('Socket(') && !headless.includes('ProcessBuilder')],
