...
const checks = [
  ...
  ['maximize/restore preserves exact pre-maximize geometry', desktop.includes('riftRestoreGeometry') && desktop.includes('currentGeometry(win)') && desktop.includes("button.setAttribute('aria-label','Restore')") && desktop.includes("button.setAttribute('aria-label','Maximize')")],
  ['core icon-only shell controls expose semantic accessibility names', filesUi.includes('id="fsBack" title="Back" aria-label="Back"') && filesUi.includes('id="fsRefresh" title="Refresh" aria-label="Refresh"') && filesUi.includes('aria-label="Browser Back"') && filesUi.includes('aria-label="Browser Reload"') && filesUi.includes('aria-label="Browser address"') && desktop.includes("min.setAttribute('aria-label','Minimize')")],
  ...
  ['Fixed-scope local agents self-activate only constructor-fixed targets inside one native action', vortexAgent.includes('ACTIVATION_TIMEOUT_MS = 3_000L') && vortexAgent.includes('context.startActivity(launch)') && vortexAgent.includes('if (lastPackage == targetPackage && root != null) return service to root') && vortexAgent.includes('actions_auto_activate') && vortexAgent.includes('internal fun ensureActiveForSession(context: Context)')],
