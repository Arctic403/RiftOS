...
const cliEvents = read(k + 'RiftCliEventBus.kt');
const cliRuntime = read(k + 'RiftCliRuntime.kt');
const cliRelayClient = read(k + 'RiftCliRelayClient.kt');
const cliRelaySettings = read(k + 'RiftCliRelaySettings.kt');
...
const relayWrangler = read('relay/wrangler.jsonc');
const cliRelayWorker = read('cli-relay/src/index.js');
const cliRelayWrangler = read('cli-relay/wrangler.jsonc');
...
const checks = [
  ...
  ['relay ignores stale socket events and does not persist payloads', (relayWorker.match(/if\s*\(\s*socket !== this\.socket\s*\)/g) || []).length >= 3 && relayWorker.includes('MAX_PENDING_REQUESTS = 64') && relayWorker.includes('readBoundedText') && relayWorker.includes('encoder.encode(raw).byteLength') && relayWorker.includes('Malformed RiftOS MCP response') && relayWorker.includes('mcp.notification') && relayClient.includes('mcp.notification') && !relayWorker.includes('ctx.storage')],
  ['RiftCLI transport is bounded and independent from MCP', cliEvents.includes('SCHEMA = "rift.cli-event/1"') && cliEvents.includes('MAX_EVENTS = 256') && cliEvents.includes('MAX_EVENT_BYTES = 96 * 1024') && cliEvents.includes('AtomicLong(System.currentTimeMillis() * 1000L)') && cliEvents.includes('fun replayAfter(afterSequence: Long)') && cliRuntime.includes('fun events(): RiftCliEventBus') && cliRuntime.includes('fun relayClient(context: Context): RiftCliRelayClient') && cliRelayClient.includes('PROTOCOL = "rift-cli-relay-v1"') && cliRelayClient.includes('cliEvents.addListener(eventListener)') && cliRelayClient.includes('"cli.replay.request"') && cliRelayClient.includes('"cli.ack"') && cliRelayClient.includes('settings.saveAckSequence(sequence)') && cliRelaySettings.includes('PREFS = "rift-cli-relay"') && cliRelaySettings.includes('TOKEN_SECRET = "rift.cli.relay.token"') && cliRelayWorker.includes('const PROTOCOL = "rift-cli-relay-v1"') && cliRelayWorker.includes('MAX_PENDING_REQUESTS = 64') && cliRelayWorker.includes('MAX_EVENTS = 256') && cliRelayWorker.includes('url.pathname === "/request"') && cliRelayWorker.includes('url.pathname === "/events"') && cliRelayWorker.includes('env.DEVICE_TOKEN') && cliRelayWorker.includes('env.DRIVER_TOKEN') && cliRelayWorker.includes('Only commands rooted at rift-cli are allowed') && cliRelayWorker.includes('type: "cli.request"') && cliRelayWorker.includes('envelope.type === "cli.event"') && cliRelayWorker.includes('type: "cli.ack"') && cliRelayWrangler.includes('"name": "rift-cli-relay"') && cliRelayWrangler.includes('"name": "RIFT_CLI_RELAY"') && !runtime.includes('RiftCliEventBus') && !relayClient.includes('"cli.') && !relayWorker.includes('cli.event') && !relayWorker.includes('notifications/riftcli') && !relayWorker.includes('getWebSockets("driver")') && !cliRelayWorker.includes('mcp.request') && !cliRelayWorker.includes('mcp.notification') && !cliRelayWorker.includes('ctx.storage')],
  ['RiftCLI Batch V2 stays bounded and separate from retired batch paths', shell.includes('MAX_CLI_BATCH_STEPS = 16') && shell.includes('MAX_CLI_BATCH_BYTES = 128 * 1024') && shell.includes('private fun parseCliBatchPlan') && shell.includes('rawToolArgs is JSONObject') && shell.includes('"rift_cli_batch" -> startCliBatch') && shell.includes('origin = "rift-cli-batch"') && shell.includes('catch (error: InterruptedException)') && shell.includes('"completed_with_failures"') && host.includes('internal fun validateCliBatchTool') && host.includes('internal fun executeCliBatchTool') && sandbox.includes('internal fun executeCliBatchRequest') && sandbox.includes('executeRequest(raw, "rift-cli-batch")') && read('src/riftshell-batch.js').includes('DISABLED: RiftShell batch commands are disabled')],
