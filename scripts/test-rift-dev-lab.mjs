...
const isAbsolute=value=>{const raw=String(value??'').trim().replace(/\\/g,'/');return raw.startsWith('/')||/^[A-Za-z]:($|\/)/.test(raw);};
const core={ready:Promise.resolve(),fs,path:{normalize,join:(...parts)=>normalize(parts.join('/')),isAbsolute,parent:path=>{const parts=normalize(path).split('/').filter(Boolean);parts.pop();return '/'+parts.join('/');}},native:{connected:true},processes:{list:()=>[]}};
...
files.set('/D:/Documents/agent-drive-source.js','window.agentDriveSource=7;');
const driveStage=await lab.executeAgentRequest({action:'stage-file',path:'src/agent-drive-source.js',sourcePath:'D:/Documents/agent-drive-source.js',cwd:'/workspace'});
assert.equal(driveStage.action,'write');assert.equal(await fs.readText('/system/devlab/stage/src/agent-drive-source.js'),'window.agentDriveSource=7;');
...
const agentSnapshot=await lab.executeAgentRequest({action:'snapshot',note:'agent snapshot'});
const agentSnapshotPaths=agentSnapshot.entries.map(entry=>entry.path);
assert.ok(agentSnapshotPaths.includes('src/agent-source.js'),'snapshot must retain the slash-root staged source');
assert.ok(agentSnapshotPaths.includes('src/agent-drive-source.js'),'snapshot must retain the D:/ staged source');
...
console.log('ok - structured local-agent requests use Dev Lab APIs, accept bare drive source paths and reject arbitrary command tunneling');
