...
assert.match(relay,/MAX_SSE_NO_DRAIN_HEARTBEATS = 4/,'idle SSE streams must have a bounded no-drain grace');
assert.match(relay,/SSE_LEASE_MS = 300_000/,'SSE subscribers must have an absolute bounded lease');
assert.match(relay,/SSE_LEASE_JITTER_MS = 60_000/,'SSE lease expirations must be jittered');
