...
- `test-semnexis-bootstrap.mjs` — executes the first QuickJS-hosted Semnexis compiler against deterministic graph/plan goldens, solver cases and invalid-source regressions.
- `test-semnexis-shell.mjs` — locks the fixed `semx` shell route, packaged compiler asset, QuickJS authority boundary and permanent absence of the retired RiftNativeToolchain/riftclang path.
