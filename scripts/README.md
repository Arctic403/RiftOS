...
- `test-semnexis-bootstrap.mjs` — executes the QuickJS-hosted Semnexis compiler against graph/plan/Native-IR goldens, `SNIRV0`, checked-i32 semantics, liveness/spill allocation, ARM32 arithmetic, explicit CFG/phi conditionals, all six signed comparison branches and explicit-state loop/backedge lowering.
