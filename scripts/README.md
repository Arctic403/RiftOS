...
- `test-semnexis-bootstrap.mjs` — executes the QuickJS-hosted Semnexis compiler against graph/plan/Native-IR goldens, frozen `SNIRV0` compatibility, derived effect/capability verification, resource budgets, liveness/spills, parallel phi copies, ARM32 arithmetic and CFG/loop lowering, including adversarial mutation tests.
- `test-semnexis-arm32-exec.mjs` — independently executes the emitted ARM32 instruction subset and differentially checks arithmetic, traps, calls, comparisons, loops and cyclic-phi parallel-copy behavior.
