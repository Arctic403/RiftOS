...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow checking into `rift-exec-v1`, executes base, Control Flow V1 and Structured Data V1 fixtures, and locks nominal struct/enum correctness, field reads, payload binding, exhaustive match, mutability, reachability and fail-closed diagnostics.
...
- `test-rift-vm.mjs` — executes/attacks the first `rift-exec-v1` Rift++ executable ABI: bounded scalar/composite ops, function calls, nominal struct/enum values, declared host imports, composite host-boundary denial, checked overflow, step limits, no eval/process path and the importable `.rift` + `main.rxe` fixture.
