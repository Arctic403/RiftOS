...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow checking into `rift-exec-v1`, executes base and Control Flow V1 fixtures on RiftVM, and locks mutability, scope, branch/loop, short-circuit, reachability, return-path and fail-closed diagnostics.
