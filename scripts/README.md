...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow/module/effect checking into `rift-exec-v1`, executes proven Gates 0–5 fixtures plus staged Gate 6A finite-`f64`/parameter-state fixtures, and locks exact transitive `storage` effects, canonical checkpoint descriptors, no implicit int↔float coercion, deterministic parameter SHA-256 identity, bounded `Vec<T,N>`, module bounds, arithmetic typing, recursion ceilings and fail-closed diagnostics.
...
- `test-rift-vm.mjs` — executes/attacks `rift-exec-v1`: bounded scalar/composite ops, Vec/Option/Result behavior, proven Gate 5 state opcodes, staged Gate 6A finite-`f64` validation and data-only `value_sha256`, negative-zero canonicalization, executable/string/depth/output limits, dotted imports, ordinary composite host-boundary denial, corrupt/schema/shape/noncanonical/oversize rejection, checked overflow, step limits, no eval/process path and the importable `.rift` + `main.rxe` fixture.
