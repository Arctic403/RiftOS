...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow/module-graph checking into one `rift-exec-v1`, executes base, Control Flow V1, Structured Data V1, Collections V1 and transitive Gate 4 module fixtures, and locks explicit alias resolution, qualified imported types/functions/enums/structs, cycle/missing/identity/ambient-module rejection, nominal data correctness, bounded `Vec<T,N>`, arithmetic typing, recursion ceilings and fail-closed diagnostics.
