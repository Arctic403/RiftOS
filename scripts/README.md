...
- `test-rift-vm.mjs` — executes/attacks the first `rift-exec-v1` Rift++ executable ABI: bounded scalar/composite ops, nominal struct/enum/vector values, raw `Vec` + `Option` / `Result` behavior, composite depth/render/public-result expansion limits, declared host imports, composite host-boundary denial, checked overflow, step limits, no eval/process path and the importable `.rift` + `main.rxe` fixture.
