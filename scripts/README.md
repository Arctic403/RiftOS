...
- `test-semnexis-bootstrap.mjs` — executes the QuickJS-hosted Semnexis compiler against graph/plan/Native-IR goldens, frozen `SNIRV0` plus additive `SNIRV1`–`SNIRV6`, `u8`, borrowed `Slice<u8>`, flat records/calls/projection/record phis, verified `u8`→`i32` widening, derived effect/capability verification, resource budgets, liveness/spills, parallel phi copies, ARM32 arithmetic and CFG/loop lowering, including adversarial mutation tests.
- `test-semnexis-arm32-exec.mjs` — independently executes the emitted ARM32 instruction subset and differentially checks arithmetic, traps, calls, comparisons, loops, cyclic/record phi copies, descriptor-backed source reads, flat-record return/call/projection ABI, variable-width token spans, record-carried token streams, parser-state execution, and native decimal accumulation over seeded external memory.
