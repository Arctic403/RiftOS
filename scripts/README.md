...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow/module/effect checking into `rift-exec-v1`, executes Gates 0–5 fixtures including the proven Gate 5 persistence fixture, and locks exact transitive `storage` effects, missing/widened/unknown authority rejection, canonical checkpoint descriptors, explicit alias resolution, module bounds, nominal data correctness, bounded `Vec<T,N>`, arithmetic typing, recursion ceilings and fail-closed diagnostics.
...
- `test-rift-vm.mjs` — executes/attacks `rift-exec-v1`: bounded scalar/composite ops, Vec/Option/Result behavior, executable/string/depth/output limits, dotted imports, ordinary composite host-boundary denial, and proven Gate 5 state opcodes with canonical descriptor validation, save/load/remove round trips, corrupt/schema/shape/noncanonical/oversize rejection, checked overflow, step limits, no eval/process path and the importable `.rift` + `main.rxe` fixture.
