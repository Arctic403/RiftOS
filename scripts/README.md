...
- `test-rift-plus-plus-core-v1.mjs` — compiles human-written `riftpp 1` Core source through lexer/parser/type/control-flow checking into `rift-exec-v1`, executes base, Control Flow V1, Structured Data V1 and Collections V1 fixtures, and locks nominal struct/enum correctness, bounded `Vec<T,N>`, `Option<T>` / `Result<T,E>`, exhaustive match, mutability, reachability, arithmetic expected-type enforcement, parser/codegen recursion ceilings, the signed-min literal edge case and fail-closed diagnostics.
- `test-riftpp-shell.mjs` — locks the normal RiftShell Core `riftpp` family, its separation from experimental RiftCLI, no-host-import execution boundary, timer/macrotask yielding, output/VM limits and atomic-batch exclusion.
- `test-rift-vm.mjs` — executes/attacks the first `rift-exec-v1` Rift++ executable ABI: bounded scalar/composite ops, nominal struct/enum/vector values, raw `Vec` + `Option` / `Result` behavior, serialized-input/aggregate-string limits, composite depth/render/public-result expansion limits, dotted declared host imports, composite host-boundary denial, checked overflow, step limits, no eval/process path and the importable `.rift` + `main.rxe` fixture.
