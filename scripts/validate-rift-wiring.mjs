...
if (!entry.includes('import("./riftpp-core.js")') || !riftppCore.includes("RIFTPP_LANGUAGE='riftpp/1'") || !riftppCore.includes("RIFTPP_CORE_VERSION='0.4.0-bootstrap'") || !riftppCore.includes('compileRiftPlusPlusCoreV1') || !riftppCore.includes('prepareRiftExecutable(executable)') || !riftppCore.includes('compileWhile') || !riftppCore.includes('compileIf') || !riftppCore.includes('compileMatch') || !riftppCore.includes('compileVecMethod') || !riftppCore.includes('MAX_TYPE_DEPTH=32') || !riftppCore.includes('MAX_PARSE_DEPTH=128') || !riftppCore.includes('blockDepth=0') || !riftppCore.includes('compileExprDepth=0') || !riftppCore.includes('expectType(left,expected,expr)') || !riftppCore.includes('structDecl()') || !riftppCore.includes('enumDecl()') || !riftppCore.includes("'+=','-=','*=','/=','%='") || !riftppCore.includes('globalThis.RiftPlusPlusCore=Object.freeze')) failures.push('Rift++ Core bootstrap compiler wiring is incomplete');
...
if (!entry.includes('yield:()=>new Promise(resolve=>setTimeout(resolve,0))') || !entry.includes('yieldEvery:512')) failures.push('Rift++ shell execution does not yield through a timer/macrotask');
