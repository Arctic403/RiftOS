...
if (!entry.includes('import("./riftpp-core.js")') || !riftppCore.includes("RIFTPP_LANGUAGE='riftpp/1'") || !riftppCore.includes("RIFTPP_CORE_VERSION='0.5.0-bootstrap'") || !riftppCore.includes('compileRiftPlusPlusCoreV1') || !riftppCore.includes('compileRiftPlusPlusCoreProgramV1') || !riftppCore.includes('linkRiftPlusPlusCoreProgramV1') || !riftppCore.includes('MAX_MODULES=64') || !riftppCore.includes('MAX_PROGRAM_SOURCE_BYTES=1024*1024') || !riftppCore.includes('linkExprDepth=0') || !riftppCore.includes('linkBlockDepth=0') || !riftppCore.includes('module linker expression nesting exceeds') || !riftppCore.includes('prepareRiftExecutable(executable)') || !riftppCore.includes('compileWhile') || !riftppCore.includes('compileIf') || !riftppCore.includes('compileMatch') || !riftppCore.includes('compileVecMethod') || !riftppCore.includes('MAX_TYPE_DEPTH=32') || !riftppCore.includes('MAX_PARSE_DEPTH=128') || !riftppCore.includes('blockDepth=0') || !riftppCore.includes('compileExprDepth=0') || !riftppCore.includes('expectType(left,expected,expr)') || !riftppCore.includes('structDecl()') || !riftppCore.includes('enumDecl()') || !riftppCore.includes("'+=','-=','*=','/=','%='") || !riftppCore.includes('globalThis.RiftPlusPlusCore=Object.freeze')) failures.push('Rift++ Core bootstrap compiler wiring is incomplete');
