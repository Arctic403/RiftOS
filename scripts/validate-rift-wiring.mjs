...
if (!entry.includes('import("./riftpp-core.js")') || !riftppCore.includes("RIFTPP_LANGUAGE='riftpp/1'") || !riftppCore.includes("RIFTPP_CORE_VERSION='0.7.0-bootstrap'") || !riftppCore.includes("SUPPORTED_PRIMITIVES=new Set(['unit','bool','u32','s32','f64','string'])") || !riftppCore.includes("PRELUDE_NAMES=new Set(['print','value_sha256'") || !riftppCore.includes("SUPPORTED_EFFECTS=new Set(['storage'])") || !riftppCore.includes("CHECKPOINT_BUILTINS=new Set(['checkpoint_save','checkpoint_load','checkpoint_remove'])") || !riftppCore.includes('parseF64(raw,node)') || !riftppCore.includes("expr.kind==='FloatLiteral'") || !riftppCore.includes("emit({op:'value_sha256'})") || !riftppCore.includes('validateEffects()') || !riftppCore.includes('stateDescriptor(type,node') || !riftppCore.includes('recursive checkpoint type') || !riftppCore.includes("this.imports.add('state.save')") || !riftppCore.includes("this.imports.add('state.load')") || !riftppCore.includes("this.imports.add('state.remove')") || !riftppCore.includes('compileRiftPlusPlusCoreV1') || !riftppCore.includes('compileRiftPlusPlusCoreProgramV1') || !riftppCore.includes('linkRiftPlusPlusCoreProgramV1') || !riftppCore.includes('MAX_MODULES=64') || !riftppCore.includes('MAX_PROGRAM_SOURCE_BYTES=1024*1024') || !riftppCore.includes('linkExprDepth=0') || !riftppCore.includes('linkBlockDepth=0') || !riftppCore.includes('module linker expression nesting exceeds') || !riftppCore.includes('prepareRiftExecutable(executable)') || !riftppCore.includes('compileWhile') || !riftppCore.includes('compileIf') || !riftppCore.includes('compileMatch') || !riftppCore.includes('compileVecMethod') || !riftppCore.includes('MAX_TYPE_DEPTH=32') || !riftppCore.includes('MAX_PARSE_DEPTH=128') || !riftppCore.includes('blockDepth=0') || !riftppCore.includes('compileExprDepth=0') || !riftppCore.includes('expectType(left,expected,expr)') || !riftppCore.includes('structDecl()') || !riftppCore.includes('enumDecl()') || !riftppCore.includes("'+=','-=','*=','/=','%='") || !riftppCore.includes('globalThis.RiftPlusPlusCore=Object.freeze')) failures.push('Rift++ Core bootstrap compiler wiring is incomplete');
