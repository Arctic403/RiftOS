...
if (n2PhaseAuthority.schema !== 'rift-memory-n2-phase-authority-v1' ||
    n2PhaseAuthority.programStatus !== 'N2.0-N2.2 PROMOTED / N2-M1 PROMOTED; N2.3 + N2.4 SOURCE-IMPLEMENTED / N2-M2 PROMOTION PENDING; N2.5-N2.12 PENDING' ||
    n2PhaseAuthority.runtimeStatus !== 'N2 CANONICAL MEMORY RUNTIME INACTIVE; N2-M2 DIAGNOSTIC ONLY' ||
    ...
    n2PhaseAuthority.phases?.slice(3, 5).some(row => row.status !== 'source-implemented' || row.promotedSourceSha !== null || row.builderRunNumber !== null) ||
    n2PhaseAuthority.phases?.slice(5).some(row => row.status !== 'pending') ||
    n2PhaseAuthority.macroImplementationPlan?.[0]?.status !== 'promoted' ||
    n2PhaseAuthority.macroImplementationPlan?.[1]?.status !== 'source-implemented/promotion-pending' ||
    n2PhaseAuthority.macroImplementationPlan?.slice(2).some(row => row.status !== 'pending') ||
...
if (!rootRoadmap.includes('N2 Federated Rift Memory Kernel — N2.0-N2.2 PROMOTED / N2-M1 PROMOTED; N2.3 + N2.4 SOURCE-IMPLEMENTED / N2-M2 PROMOTION PENDING; N2.5-N2.12 PENDING; CANONICAL MEMORY RUNTIME INACTIVE') ||
