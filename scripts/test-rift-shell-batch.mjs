...
const context={window:{RiftOSCore:{fs}},console,Math,Date};context.window.window=context.window;Object.assign(context,context.window);vm.createContext(context);
...
const execute=async command=>{
  ...
  if(cmd==='fail')throw new Error('planned failure');
};

await assert.rejects(()=>context.window.RiftShellBatch.run('write original.txt changed ; mkdir temp ; fail',{state,execute,resolve,print:value=>output.push(String(value))}),/rolled back/);
