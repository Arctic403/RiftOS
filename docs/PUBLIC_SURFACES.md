...
| `RiftLlmBridge` | `riftllm-bridge.js` + `RiftLlmDevClient.kt` | Trusted-shell optional bridge to the standalone RiftLLM APK's fixed token-gated Binder Dev API; owns exact source sync and guarded Workspace publication orchestration plus bounded local-only RiftCorpus build/status under `/workspace/RiftLLM/tokenizer/private/`, never arbitrary process execution or RiftLLM-owned Dev Lab state. |
