...
| `RiftLlmBridge` | `riftllm-bridge.js` + `RiftLlmDevClient.kt` | Trusted-shell optional bridge to the standalone RiftLLM APK's fixed token-gated Binder Dev API; owns exact source sync and guarded Workspace publication orchestration, never RiftLLM-owned Dev Lab state. |
