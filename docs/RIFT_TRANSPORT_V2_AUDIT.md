# Rift AI / RiftBrowser Transport V2 Audit
...
- Add strict `rift-tools-v2` JSON requests with correlated request/call IDs, up to eight sequential calls, and one `RIFT_TOOL_RESULT_V2` continuation packet.
- Retain legacy `<rift_call>` support.
...
ChatGPT receives a compact project descriptor, searches/reads only necessary paths or ranges, and returns JSON operations. RiftOS applies edits locally and returns bounded results/diffs. Whole projects and generated ZIP bytes do not travel through the ChatGPT composer.
