...
`RiftMemoryN2M4SelfTest.kt` uses a separate app-private `n2-m4-proof.sqlite` and independently returns `n2_7` and `n2_8` evidence. N2.7 fixtures cover procedural transfer, recurrence, evidence-backed causal confidence, rejection of weak causal source, open-commitment restart and protected-policy overwrite resistance. N2.8 fixtures cover all four router modes, all seven retrieval lanes, graph dependency retrieval, similarity-vs-evidence precedence, cross-project exact-query isolation, bounded minimal context, fail-closed insufficient context, NO mode and constructor bounds. The diagnostic is read-only through exactly three `rift_info` lanes, reports `runtimeAuthority=false`, and is absent from Local Agent/RiftCLI authority. `scripts/test-rift-memory-n2-m4-v1.mjs` and the exact Gradle source snapshot permanently gate the source contract. `riftmemory/n2-phase-authority.json` marks N2.7, N2.8 and N2-M4 `source-implemented` with null promoted source/run. Promotion requires Builder/source gates, installed M4 diagnostics, SQLite integrity, clean Observer continuity and separate N2.7/N2.8 evidence. FAIL-009 then proved that direct machine-authority consumers can also be maintained validators: `validate-rift-docs.mjs` froze the pre-M4 lifecycle while the proof closure scanned only test paths. Config-read impact now includes bounded maintained test/validate/verify scripts, and the semantic-impact regression requires contract + M1 + M2 + M3 + M4 + `validate-rift-docs.mjs` as the current direct phase-authority consumer set.
