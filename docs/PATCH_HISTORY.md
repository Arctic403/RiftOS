...
N2.12 is **SOURCE IMPLEMENTED / PROMOTION PENDING**. `RiftMemoryN2FinalSelfTest.kt` is a diagnostic-only evidence aggregator over the already-promoted M1-M6 runtime diagnostics. It does not introduce another memory engine or authority path. It re-runs M1-M6, maps their concrete outputs onto all 12 frozen `n2-contract-v1.json` correctness scenarios, computes the eight zero-tolerance outcomes, emits weakest-link categories, exposes the immutable contract/corpus/store/threshold hashes, records installed `BuildConfig` source/run continuity, and separates ARM32 installed-device proof from ARM64 build/correctness parity. The final diagnostic keeps `runtimeAuthority=false`, explicitly reports that comparative/performance benchmarks were not executed, and carries the global rule that such benchmarks remain deferred until the entire RiftCLI roadmap is complete and live.

`scripts/test-rift-memory-n2-final-v1.mjs` hashes the actual frozen contract file, verifies all six immutable identity hashes, exact 12 corpus IDs, exact eight zero-tolerance keys, dual `arm64-v8a` + `armeabi-v7a` Gradle wiring, BuildConfig source/run fields, mandatory Android ownership, exactly three read-only `rift_info` attachments, inactive Local Agent/RiftCLI authority, and absence of timing/benchmark execution APIs. Machine authority marks only N2.12 `source-implemented` with null promoted source/run; N2.0-N2.11 remain promoted and canonical memory runtime remains inactive. Builder/install/device evidence is still required before N2.12 may promote; N3 and batch re-exposure remain blocked until that final correctness gate closes.
