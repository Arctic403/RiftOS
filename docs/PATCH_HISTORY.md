...
Recovery authority remains gated correctly: list/poll/cancel retain their disabled-state observation/control exception, while `rift_cli_job_recover` is not classified as disabled-state job control because `resume` can execute authority and therefore requires the process-local `CONFIRM-EXPERIMENTAL` enable gate. The recovery command remains absent from Local Agent and MCP, and Batch V2 itself remains externally unexposed. Process-loss reclassification now preserves the pre-crash recovery/step journal instead of replacing it, which is required for exact B2A decisions. Local Observer on the dirty B2A source is clean after adding the new source-ownership entry: Claims clean, Integrity clean, Consistency zero findings, Contracts clean, seeded Propagation complete, and Proofs deep with 8 obligations / 0 unresolved. External Node/Gradle Builder validation plus installed-device recovery proof remain required before B2A promotion. B2B failure/rollback semantics remains next.
