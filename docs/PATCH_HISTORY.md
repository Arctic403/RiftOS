...
N2-M5 is **SOURCE IMPLEMENTED / PROMOTION PENDING**. `RiftMemoryObserverValidatorLoopV1.kt` accepts bounded normalized Observer/Validator snapshots as evidence only, writes those observations through canonical `RiftMemoryCandidateV1` + `RiftMemoryReconciliationV1`, derives discrepancy records without granting them authority, rejects incomplete/cross-scope/stale/replayed inputs, and updates state only through normal reconciliation. Verified Validator evidence can reopen a false `COMPLETED` commitment to `OPEN`; the adapter never directly mutates trusted truth and remains backend-neutral.

`RiftMemoryN2M5SelfTest.kt` uses separate app-private `n2-m5-proof.sqlite` diagnostics and covers incomplete evidence, stale-state correction, stale/replay blocking, failed validation reconciliation, false-completion reopening, project isolation, exact bounds, persisted difference evidence, close/reopen recovery, evidence-backed authority and SQLite integrity. It is exposed read-only through exactly three `rift_info` lanes with `runtimeAuthority=false`; Local Agent/RiftCLI do not reference the adapter or self-test. Machine authority marks N2.9 and N2-M5 `source-implemented` with null promoted source/run; Builder/install/device proof remains required before promotion. No comparative/performance benchmark is part of M5.

## N2-M5 — N2.9 Observer/Validator closed loop

N2-M5 is **SOURCE IMPLEMENTED / PROMOTION PENDING**. N2.9 adds `RiftMemoryObserverValidatorLoopV1.kt` as a bounded evidence adapter over the existing canonical model and N2.3 reconciliation path. Observer snapshots and Validator results become explicit evidence observations; neither subsystem receives direct trusted-truth mutation authority. Incomplete evidence may be retained provisionally for audit but is forbidden from updating target truth. Complete evidence is bounded by subject/value/details/snapshot caps, project scope, source-state SHA-256 and monotonically increasing source sequence; stale/replayed snapshots and cross-project corrections fail closed.

State correction follows the frozen loop: Planner/Tools state → Observer/Validator evidence → persisted observation → explicit discrepancy/difference → `RiftMemoryReconciliationV1` → canonical transaction. Strong verified evidence can supersede weaker provisional planner state through the existing authority rules. Validator-backed commitment correction can reopen a false `COMPLETED` commitment as `OPEN` using verified evidence and the original `resumeKey`; the adapter does not bypass reconciliation or grant itself protected policy/config authority.

`RiftMemoryN2M5SelfTest.kt` uses separate app-private `n2-m5-proof.sqlite` and proves incomplete-evidence fail-closed behavior, fresh Observer stale-state correction, stale/replay rejection, failed Validator correction, false-completion reopening, project isolation, payload bounds, durable discrepancy records, close/reopen recovery and SQLite integrity. It remains `diagnosticOnly=true`, `runtimeAuthority=false`, and is surfaced read-only through exactly three `rift_info` lanes. `scripts/test-rift-memory-n2-m5-v1.mjs` permanently gates the source contract, ownership, lifecycle, package wiring and inactive Local Agent/RiftCLI authority. `riftmemory/n2-phase-authority.json` marks N2.9 and N2-M5 `source-implemented` with null promoted source/run; N2.10-N2.12 remain pending and canonical runtime remains inactive.
