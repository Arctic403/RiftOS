...
The transport validator now freezes the exact 24-tool family plus the Local-Agent-only Batch routing. `test-rift-cli-batch-v2.mjs` requires the five public controls, max-16 schema, non-recursive controls and private internal RiftCLI names. The Contracts oracle recognizes the explicit Local Agent dispatch path; the compatibility method routing tags remain fail-closed because `callAsyncInternal` intercepts Batch controls before sandbox dispatch. Builder run `36206957187` on source `d5123d945766b20b6c3fd64ed70b1211a15497c3` reached the cross-boundary regression after the 24-tool transport and docs gates passed, then FAIL-020 exposed one stale `toolDefs.size == 19` assertion. That regression expectation is repaired to 24 in source; Builder confirmation remains pending. Kotlin/Gradle, install, ChatGPT tool-catalog resync and direct MCP submit/list/poll/cancel/recover live proof are still required before this phase can be promoted.
