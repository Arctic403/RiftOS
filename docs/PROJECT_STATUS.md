...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; Patch 10.35 semantic-order determinism hardening is source-implemented but N1.8.0 is not promoted.** Patch 10.33 fixed repository byte/content truth and installed source `7ee74c5034bb14c30945d28971c56f35424e5301` re-ran the original same-size source edit, README content-only edit and delete/recreate kill cases successfully. Patch 10.34 then moved PI persistence to cache schema v4 with exact content SHA plus producer provenance (`RiftSourceIntelligenceV2.VERSION` + trusted full `BuildConfig.RIFT_SOURCE_SHA`). Installed source `1c0e644f7424d30f20acdc1da5cf540833802ff2` proved old-cache rejection and, after a true Android force-stop/reopen, correctly reported `cacheLoadStatus=loaded`, no rejection reason, analyzer version 2 and the exact trusted source SHA. The unchanged clean repo nevertheless changed canonical consistency graph from `93bc7228ea90278bbb1d2421bad89850c1422c590bdd0113698f4f24de06736b` to `e3868cc6f0a260036a7383d5a78fa26eedb805d97e3fdb191f0e9d3856241dbd` while counts stayed 1017 facts / 1072 edges / 258 files. Audit traced that restart-only divergence to traversal-order-sensitive PI-v2 dependency fallbacks: Python and generic symbol resolution used `firstOrNull`, so cache reload ordering could silently select a different target. Patch 10.35 canonicalizes graph iteration, adds unambiguous package-qualified Kotlin/Java import matching, and makes ambiguous Python/symbol candidates fail closed to unresolved rather than picking an arbitrary edge. Permanent regression coverage forbids those first-match fallbacks. Local Android source validation is green; Builder/Kotlin compile, install and true restart parity are required before torture resumes. The staged subsystem planner remains blocked until this hardened foundation passes. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
