...
- **N1.8.1 syntax/import integrity is hardening as Patch 10.45 and remains unpromoted.** The first installed Patch 10.44 run proved the v5→v6 cache rebuild path but also exposed three promotion-blocking false-positive classes: generic text rows were incorrectly treated as missing syntax evidence; valid Kotlin/JavaScript source produced structural false errors around Kotlin interpolation and JavaScript regex/template constructs; and Kotlin/Java package locality incorrectly treated repository path text such as `android/app` as proof that SDK imports were local. Patch 10.45 advances the shared analyzer to v4, PI persistence to schema v7, and syntax evidence to conservative `bounded-structural-v2-conservative`, which masks constructs the lightweight scanner cannot safely reason through rather than inventing errors. Kotlin/Java qualified imports now resolve only against inferred source-package roots and package-local symbols, preventing `android.app.Activity` false-local findings while allowing same-name types such as `com.codynex.editor.EditorState` to resolve to the correct package. JavaScript static `from` extraction is statement-anchored so source text strings containing `from './x'` do not become fake dependencies. `not-applicable` syntax rows no longer make integrity incomplete. The permanent Builder regression `scripts/test-rift-integrity-v1.mjs` now locks these exact live failures. The N1.8.0 canonical observer remains unchanged and locally re-proves complete/clean after the hardening edits. Promotion still requires the rebuilt APK plus cold/warm/restart, v6→v7 cache migration, syntax false-positive/adversarial fixtures, local/external/ambiguous dependency fixtures, rename/delete/move drift and focused-vs-oracle parity.
