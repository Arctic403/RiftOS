...
- Native RiftCLI Gate N0 is proven on-device and Gate N1 Driver Protocol is the active implementation gate. The canonical CLI core remains C++ under `android/app/src/main/cpp/riftcli/`; `RiftCliHost.kt` remains a thin JNI transport host only.
...
- Gate N0 proved the minimal native bootstrap on-device. Gate N1 now adds `rift-cli driver request`: while explicitly enabled, the C++ core may authorize one RiftOS shell action or one direct `rift_*` ToolHost action per request. Every authority-bearing request has a replay-protected request-id; both lanes submit live-poll jobs instead of blocking; jobs can be listed/recovered by request-id, polled or cancelled; direct model/API clients remain absent and driver continuation stays external and bounded.
