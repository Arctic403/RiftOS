...
- **N1.8.5 proof obligations and focused verification is SOURCE-IMPLEMENTED and promotion-pending.** `RiftProofObligationsV1.kt` adds the read-only `project kind=proofs` planner over exact candidate-impact evidence. It emits deterministic evidence-linked proof obligations, selects directly evidenced affected tests separately from path-affinity supplemental tests, escalates to deep verification for incomplete impact evidence, API/build/deletion/multi-project risk, and records unresolved obligations instead of substituting unrelated or general tests. The planner is evidence-only and executes no verification. Permanent regression, Gradle mandatory-source coverage, source ownership and `npm check` wiring are present; Builder compilation, signed-DEX proof, installed semantic fixtures, bounds/determinism/restart proof and N1.8.0-N1.8.4 continuity remain required before promotion.
