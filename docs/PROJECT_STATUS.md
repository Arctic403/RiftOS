...
- **N1.8.5 proof obligations and focused verification is SOURCE-IMPLEMENTED and promotion-pending.** `RiftProofObligationsV1.kt` adds the read-only `project kind=proofs` planner over exact candidate-impact evidence. It emits deterministic evidence-linked proof obligations, selects directly evidenced affected tests separately from path-affinity supplemental tests, escalates to deep verification for incomplete impact evidence, API/build/deletion/multi-project risk, and records unresolved obligations instead of substituting unrelated or general tests. The planner is evidence-only and executes no verification. The first installed proof attempt exposed a Workspace Records hot-path blocker rather than a planner defect: semantic impact was forcing full-workspace reconciliation and replaying a 2,000-record history. Source `1ee0437d9bfa266039a09773db3a27176e28bb0e`, Builder run 332, proved the Workspace Records repair itself was working (`rift_workspace_diff`: 0 changed files / 0 records / 0 patch sessions) but `project kind=proofs` still timed out. The remaining blocker was in `candidateImpact()`, which refreshed the entire workspace PI index before inspecting the candidate. Current source now derives affected project roots first, skips PI refresh entirely for `no-candidate-changes`, and refreshes only bounded `candidate-projects` for real candidates. The 256-record rolling history, tracking-policy-v2 exclusions/rebaseline, persisted candidate sessions, watcher-aware currentness, 60-second Workspace Records budget, 75/90/100/110/120-second request ladder and SSE heartbeat/abort/backpressure + 300-360 second lease remain intact. Permanent regressions now explicitly forbid whole-workspace candidate-impact refresh; Builder compilation, signed-DEX proof, installed zero-change/candidate-scoped semantic fixtures, bounds/determinism/restart proof and N1.8.0-N1.8.4 continuity remain required before promotion.
