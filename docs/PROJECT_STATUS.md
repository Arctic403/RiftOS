...
- Rift++ Core `0.6.0-bootstrap` is installed/device-proven through Gate 5 on RiftOS source commit `972206a0d4446811572aa697e0f69cf46097dc8c`: exact transitive `allow [storage]` effects, bounded `checkpoint_save/load/remove`, canonical checkpoint type descriptors, and explicit `state.save/state.load/state.remove` RiftVM imports. The installed RiftLLM+ two-launch proof restored complete BrainState across a real RiftRT close/relaunch boundary and verified post-clear fallback. State authority remains app-private behind the existing RiftApp/RiftRT `storage` declaration; no global kernel capability, MCP tool, generic filesystem authority, or RiftCLI route was added. RiftLLM+ Gate 5.5 then stress-tested verified memory across 33 RiftRT sessions without changing the language. Gate 6A numeric/parameter primitives is next; parameter learning remains unproven until changed parameters improve frozen unseen challenges beyond exact-memory retrieval. Core stays separate from the non-executable RiftCLI/V0 swarm DSL.
