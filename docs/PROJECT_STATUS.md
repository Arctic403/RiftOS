...
- Native RiftCLI Gates N0 and N1 are proven on-device; N1.5 persistent push and N1.6 Batch V2 are source-complete and locally gated, with new Builder/APK/device promotion still pending. The canonical CLI core remains C++ under `android/app/src/main/cpp/riftcli/`; `RiftCliHost.kt` remains a thin JNI transport host only.
