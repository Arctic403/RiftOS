...
- **N1.8.4 documentation/roadmap/TODO claim verification is ACTIVE with a hard source-of-truth rule.** Current source, manifests, build configuration and other machine-readable repository state are authoritative for implementation/configuration claims; exact Builder artifacts and installed-device/runtime evidence are authoritative for build/install/promotion claims. README/docs, ROADMAP, TODOs, status prose and comments are non-authoritative claim surfaces only. If they disagree with stronger evidence, the claim surface is stale/unsupported; the Observer must never reinterpret or mutate source to satisfy documentation, and incomplete authority must remain unresolved instead of trusting prose as fallback. Historical documents are exempt from current-state contradiction findings only when explicitly/deterministically classified as historical. N1.8.4 runtime implementation is not started yet.
