...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; N1.8.0 is source-implemented but not yet promoted.** `RiftRepositoryConsistencyObserver.kt` derives a bounded repository fact graph from the existing PI-v2 graph with stable fact/edge/finding identities, separate content hashes, deterministic canonical graph SHA-256, explicit completeness/bounds and a verified rebuildable app-private cache. The existing Code Mode `project` operation now has a read-only `kind=consistency` view; no second source/symbol index is introduced. `test-rift-repository-consistency-v1.mjs` is in the main Builder check chain. Local Android source validation is green. The first installed full-repo proof returned `complete=false` because the observer inherited PI-v2's public 120-file/600-edge preview caps. Source now routes consistency through the same PI-v2 builder with 1024-file/1024-edge internal bounds; the current repo's 258 files / 758 dependencies fit with headroom. **Promotion is now gated by a full installed torture matrix rather than one happy-path scan:** cold/warm deterministic repeats, tracked content-only mutations, add/delete/rename/move/copy, cache corruption/recovery, process restart, concurrent reads, mutation-during-scan races, exact boundary/overflow fixtures, path/identity stress, file-type/exclusion/oversize handling, dependency-shape stress, resource pressure, compact/full result parity and clean-oracle differential checks. Any false-clean, silent skip/truncation, nondeterminism, stale cache, stale node, mixed-state race, hang/crash or observer authority mutation blocks promotion and becomes a permanent regression case. **The staged subsystem scan planner is design-locked but implementation-blocked until N1.8.0 passes that torture gate.** After promotion, normal scans should start with the owning subsystem/domain and expand only across affected graph/proof edges, while periodic or uncertainty-triggered full-repo rebuilds remain the correctness oracle. N1.8.1+ are still pending. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
