...
- Rift++ Core bootstrap frontend `0.3.0-bootstrap`: bounded `riftpp 1` parsing/type checking and deterministic `.rxe` lowering with functions, typed locals, mutable `var`/assignment, lexical/control flow, nominal local `struct`/`enum` values, struct field reads, enum payload construction/binding, exhaustive `match` on enums/bool, checked arithmetic and all-path return analysis; separate from the non-executable RiftCLI/V0 swarm DSL.
