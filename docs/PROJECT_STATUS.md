...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; installed source `ce8fbbd3f7179c8b153f44daaa2aedb6f2734ea4` has closed the Patch 10.38 warm-cache blocker, but N1.8.0 is not yet promoted.** Current installed proof is clean at 1034 facts / 1088 edges / 260 repository files with canonical graph SHA-256 `2551bb5c0ee03b329bc7de068b9afdbc06d98c39f94839f7b5e5b2ba148aee25`; ten consecutive warm scans, three concurrent consistency reads beside an ordinary PI graph read, and cache state all remained deterministic. Fresh disposable semantic fixtures proved exact 128 MiB cold+warm complete while 128 MiB + 1 fails closed cold+warm with `semantic-total-byte-bound`; observer propagation is exact=`complete=true`, +1=`complete=false` with `semantic-evidence-incomplete`. Earlier installed Patch 10.37 evidence still covers v5 restart integrity, long stable keys, >4 MiB snapshot fail-soft behavior, 4096/4097 symbol/dependency bounds, resolver bounds, path/identity stress and mutation-during-scan coherence. Patch 10.42 strengthens the permanent Builder regression for the remaining app-private cache write/post-write verification failure paths: it now proves fail-soft control-flow ordering, `persisted=false/verified=false`, cleanup, and no throw before success. Runtime observer code is unchanged. Remaining promotion work is Builder execution of Patch 10.42 plus the exact-current-build process-boundary/restart confirmation; N1.8.1+ and all further CLI intelligence remain blocked until N1.8.0 promotion. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
