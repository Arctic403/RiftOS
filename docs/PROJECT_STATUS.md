...
- Native RiftCLI has been reset to Bootstrap-0. The canonical CLI core is now C++ under `android/app/src/main/cpp/riftcli/`; `RiftCliHost.kt` is a thin JNI transport host only.
...
- Bootstrap-0 exposes only help/status/architecture plus explicit process-local enable/disable. Even when enabled it has no mutation, tool, network, model/API, project-memory, planner, verification or Local Agent authority.
...
- The permanent reasoning dependency is external driver → MCP/RiftShell → RiftCLI. RiftCLI does not call a model or inference API. Future professional-engineering capabilities are rebuilt as gated native C++ subsystems documented in `docs/systems/riftcli/README.md`.
...
- `semx` routes through the same bounded headless QuickJS owner using the separately packaged Semnexis bootstrap asset. Semnexis 0.6 is installed-device proven on source `1d743b6fda2f5e7f1085186bc4bf6e9bbbd3ef36`, including checked add/sub/mul/div, `r4-r7` allocation, CFG/phi conditionals and explicit-state loop backedges. The 0.6.1 hardening layer is source verified: IR effects/capabilities are re-derived, runtime ELF verification is bound to canonical IR lowering, cyclic phi copies use parallel-copy resolution, compiler/AST/CFG/artifact/output budgets are enforced, and Builder runs independent ARM32 execution plus the exact embedded `semx self-test`. Current 0.7.0 source advances the self-hosting pressure loop with real `u8`, frozen/additive `SNIRV1`, bounded generic type-reference syntax, borrowed read-only `Slice<u8>`, `slice_len`, bounds-checked `slice_get`, `SNIRV2`, flat immutable records in `SNIRV3`, direct/multiword r0-r3 record returns, record-returning calls, field projection in `SNIRV4`, record-valued `if`/`phi.record` merges and record-carried loop state in `SNIRV5`, verified `u8`→`i32` widening in `SNIRV6`, variable-width numeric token spans, native decimal accumulation, and a streaming parser-state kernel. The current promotion gate is `semnexis-bootstrap-self-test/13`; 0.7 remains source + independent-machine-regression verified until that APK/device gate passes.
