...
- **N1.8 Repository Consistency Observer is architecture-locked and implementation-pending as a hard pre-N2 gate.** Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`. It reuses Workspace Records + PI-v2 and adds a rebuildable fact/claim graph, incremental invalidation/reverse dependency propagation, code/import/reference/contract consistency, docs/README/ROADMAP/TODO claim checking, proof obligations, clean-vs-incremental parity and adversarial precision/recall/latency/memory benchmarks. It remains evidence-only.
