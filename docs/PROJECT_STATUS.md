...
- **N1.8.1 syntax/import integrity is hardening as Patch 10.47 and remains unpromoted.** Installed Patch 10.46 successfully eliminated the remaining dynamic-import false positives (`localMissing=0`) and rejected/rebuilt v7→v8, but exposed a single structural-v3 off-by-one contract bug: `javascriptRegexEnd()` returns the index immediately after a regex literal, while the structural caller still resumed at `regexEnd + 1`. That skipped the token immediately after every regex—commonly the closing `)` in `assert.match(..., /regex/)`—and exploded the live oracle to 922 findings, 31 invalid files and `syntax-issue-bound`. Patch 10.47 changes the caller to resume exactly at `regexEnd`, advances the analyzer to v6, PI cache to v9 and syntax mode to `bounded-structural-v4-conservative`, and permanently asserts that `index = regexEnd + 1` cannot return. The known-broken installed v5 runtime also propagates `syntax-issue-bound` into the N1.8.0 consistency completeness result while scanning this workspace; there are still zero repository-consistency findings. Therefore exact N1.8.0 recovery and N1.8.1 cleanliness must be proven after the corrected v6/v9 APK is installed. Android source validation is green.
