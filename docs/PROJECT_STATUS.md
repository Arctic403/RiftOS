...
- Rift++ Core `0.5.0-bootstrap` remains installed/device-proven through Gate 4. Local source now stages Core `0.6.0-bootstrap` Gate 5: exact transitive `allow [storage]` effects, bounded `checkpoint_save/load/remove`, canonical checkpoint type descriptors, and explicit `state.save/state.load/state.remove` RiftVM imports. Gate 5 is not promoted yet: CI/build plus installed RiftRT cross-launch persistence proof remain required. State authority stays app-private behind the existing RiftApp/RiftRT `storage` declaration; no global kernel capability, MCP tool, generic filesystem authority, or RiftCLI route was added. Gate 6 remains blocked until Gate 5 promotion, then targets numeric/parameter primitives and a real parameter-update path; RiftLLM+ will not call that parameter learning until changed parameters improve frozen unseen challenges beyond exact-memory retrieval. Core stays separate from the non-executable RiftCLI/V0 swarm DSL.
