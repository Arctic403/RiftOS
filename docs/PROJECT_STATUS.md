...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; content-truth + semantic-cache provenance hardening is source-implemented but N1.8.0 is not promoted.** Installed source `5de7f5065160b2bbe263e8cc0d179f6099af4347` proved full-repo `complete=true`, 1017 facts / 1072 edges and ten identical warm hashes, then exposed the original content-truth defect. Patch 10.33 added exact per-file SHA/size + semantic status and content-bound Repository Fact Graph v2 file facts. Installed source `7ee74c5034bb14c30945d28971c56f35424e5301` re-ran the three original failures successfully: same-size source content edit, README content-only edit and delete/recreate-at-same-path with unrelated bytes all changed canonical repository state correctly. The second-round review then found a separate stale-semantics risk: persisted semantic entries were not bound to the analyzer/build that produced them. Source now uses PI cache schema v4 with producer provenance (`RiftSourceIntelligenceV2.VERSION` + trusted full `BuildConfig.RIFT_SOURCE_SHA`), rejects missing/mismatched/untrusted producer caches, denies trusted persistent semantic reuse to local/unknown build identities, and exposes cache load/rejection diagnostics. Local Android source validation is green. External Builder/Kotlin compile plus installed proof of old-cache rejection/rebuild and current-producer load after restart are required before the remaining torture matrix continues. The staged subsystem planner remains blocked until this hardened foundation passes. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
