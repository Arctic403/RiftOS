...
- **N1.8.1 syntax/import integrity is source-implemented as Patch 10.44 and remains unpromoted pending Builder/install/torture.** The shared source analyzer is v3; PI persistence is schema v6 and carries bounded structural syntax evidence plus dependency local-intent metadata. A new read-only `project kind=integrity` lane is deliberately separate from the promoted N1.8.0 graph: blank query runs the clean full-repository oracle, focused query seeds a bounded scan with direct outgoing/reverse frontier, `complete` is separate from `clean`, and the exact evidence is bound by `integritySha256`. Deterministic findings are emitted only for bounded structural syntax failures, provably missing local dependencies, or ambiguous local candidates; external/unclassified imports are not converted into fake errors. The permanent Builder regression is `scripts/test-rift-integrity-v1.mjs`. Promotion requires compiled Builder proof plus installed cold/warm/restart, v5→v6 cache migration, syntax false-positive stress, local/external/ambiguous dependency fixtures, rename/delete/move drift and focused-vs-oracle parity. The N1.8.0 canonical observer remains unchanged until N1.8.1 itself is promoted.
