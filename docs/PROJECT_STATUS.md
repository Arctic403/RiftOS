...
- **N1.8.0 Repository Consistency Observer foundation is PROMOTED on installed source `9d196567e38e781d97a24bb2c808b47cbc2303eb`.** The strengthened Patch 10.42 Builder gate passed, then the exact installed build stabilized at `complete=true`, 1034 facts / 1088 edges / 260 repository files, graph SHA-256 `25869f703a8a4a7fe36b28ae6f9c3ab34daece15925c21b98166100eaab57d87`, verified cache, and `changed=false`. After a real Android force-stop/reopen, the **first** consistency read reproduced that exact SHA, graph ID, counts, completeness and cache state; RiftCLI also returned to `enabled=false`, proving the process-local non-persistent gate reset. Earlier installed torture evidence covers v5 cache integrity/recovery, long stable-key identity, >4 MiB snapshot fail-soft behavior, 4096/4097 symbol and dependency bounds, resolver bounds, mutation sensitivity, mutation-during-scan coherence, path/identity stress, eviction/rebuild, concurrency/reentrancy, exact/+1 semantic and repository byte bounds, and safe result omission. Patch 10.42 permanently locks cache write/post-write verification fail-soft control flow in the Builder chain. The N1.8.0 freeze is therefore cleared, but no Planner/Memory/Skills lane is activated automatically; N1.8.1 is now source-implemented with promotion pending Builder/install/torture, N1.8.2-N1.8.7 remain pending, and full N1.8 promotion is still required before N2. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
- **N1.8.1 syntax/import integrity is hardening as Patch 10.46 and remains unpromoted.** Installed Patch 10.45 reduced the first live failure set from 176 findings to 37, restored `complete=true`, eliminated the SDK-package false-local flood, and cleanly rejected/rebuilt v6→v7; however 9 valid source files still produced 34 structural false positives and three remaining `local-missing` findings came from literal assertion strings such as `includes('import("./riftpp-core.js")')`, not executable imports. Patch 10.46 advances the shared analyzer to v5, PI persistence to schema v8, and syntax mode to `bounded-structural-v3-conservative`. It replaces the retired last-quote Kotlin interpolation heuristic with bounded interpolation-expression matching, recursively masks valid JavaScript template literals including nested templates/interpolations while preserving newlines, and adds a whole-text JavaScript code-position mask so dynamic `import()`/`require()` dependencies are accepted only when the call token occurs in executable code rather than strings/comments/templates/regex. Valid but successfully parsed Kotlin interpolation/template spans are masked; malformed/unterminated constructs remain unmasked so the structural scanner can still report them. The permanent Builder regression now locks the recursive template/Kotlin helpers, code-position filter, analyzer v5/cache v8, structural-v3 mode and the absence of the retired last-quote heuristic. N1.8.0 remains clean locally at 1034 facts / 1088 edges / 261 files, a verified warm consistency read with `changed=false` before the documentation-only edits. Promotion still requires Builder/install, v7→v8 migration, full live integrity rerun and the remaining syntax/import/path-drift/frontier parity torture.
