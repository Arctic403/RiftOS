...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; content-identity hardening is source-implemented but N1.8.0 is not promoted.** Installed source `5de7f5065160b2bbe263e8cc0d179f6099af4347` proved full-repo `complete=true`, 1017 facts / 1072 edges, ten identical warm hashes, structural mutation tracking, dependency-target changes, concurrent reads and path identity. The torture fixture then proved a content-truth defect: content-only source/doc edits and delete/recreate-at-same-path could leave or restore the same repository graph/file-fact identity even when real file bytes changed. Source now uses PI-v2 cache schema v3 with exact per-file SHA/size + semantic status, keeps semantic and repository-file evidence separate, forces content verification for consistency, removes stale semantics for unindexable files, invalidates both evidence layers together on mutations/rollback, and feeds Repository Fact Graph/cache v2 content-bound file facts. Local Android source validation is green. External Builder/Kotlin compile and a full installed torture restart are required. The staged subsystem planner remains blocked until this hardened foundation passes. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
