...
- Rift++ Core bootstrap frontend `0.5.0-bootstrap` is installed/device-proven through Gate 4: Gates 0–3 plus bounded compile-time `use` module graphs, explicit/default aliases, qualified cross-module functions/types/structs/enums, cycle/identity/missing-module rejection, 64-module / 1 MiB graph bounds, and deterministic single-`.rxe` linking with zero runtime authority expansion. Gate 5 capability/effect + bounded persistence remains next. Gate 6 then targets numeric/parameter primitives and a real parameter-update path; RiftLLM+ will not call that parameter learning until changed parameters improve frozen unseen challenges beyond exact-memory retrieval. Core stays separate from the non-executable RiftCLI/V0 swarm DSL.
