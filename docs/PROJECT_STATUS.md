...
- **N1.8 Repository Consistency Observer is a hard pre-N2 gate; Patch 10.36 PI-cache integrity hardening is source-implemented but N1.8.0 is not promoted.** Installed Patch 10.35 source `8ecc5433dcca153a669b09f942bc91e64965c0fa` fixed the restart-order dependency divergence and live-proved exact true-restart graph parity at `d9064a2ba8f1742116aa0f88e67110a5022907a9bf699bab9d61f2a27e7109d7` with 1017 facts / 1071 edges / 258 files, ten identical warm runs, concurrent/reentrant reads, mutation-snapshot coherence, path/content identity, ambiguity fail-closed resolution, package-qualified Kotlin resolution, exact 1024-file and 1024-dependency boundaries, exact/above 2 MiB semantic-file behavior, metadata-only evidence and 32-project observer-cache eviction/rebuild. Continued torture audit found a new hard blocker in PI cache v4: cached semantic rows were producer-bound and file-SHA-bound but the persisted `symbols`/`dependencies` payload itself had no integrity seal, so a syntactically valid corrupted row with the correct file SHA could be reused. Patch 10.36 upgrades PI persistence to cache schema v5 with a canonical SHA-256 whole-payload seal; missing or mismatched seals fail closed before producer validation or semantic-row load. Permanent regression coverage locks the v5 seal and validation order. Local Android source validation is green; Builder/Kotlin compile, install, v4→v5 rejection/rebuild, true restart v5 reload and canonical graph parity are still required. The staged subsystem planner remains blocked until this hardened foundation passes. Canonical spec: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
