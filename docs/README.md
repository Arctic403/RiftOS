...
| Repository Consistency Observer | **N1.8.0 PATCH 10.36 / TORTURE GATE PENDING** | [`systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`](systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md) | Installed Patch 10.35 (`8ecc5433…`) live-proved true-restart graph parity, ambiguity fail-closed behavior, exact bounds, mutation coherence, path/content identity, concurrency and observer-cache eviction/rebuild; continued torture found PI cache v4 semantic payloads lacked an integrity seal, so Patch 10.36 upgrades to cache v5 with canonical SHA-256 whole-payload verification before semantic rows load; Builder/install/restart v5 proof still required; staged subsystem planner remains blocked |
...
| N3 Architecture/Impact Engine | **N3.0 PROMOTED / N3-M1 PI BRIDGE INSTALL PENDING** | [`systems/riftcli/N3_ARCHITECTURE_IMPACT_ROADMAP.md`](systems/riftcli/N3_ARCHITECTURE_IMPACT_ROADMAP.md) | Run-390 contract promoted; one hidden read-only PI bridge is source-implemented before N3.1+ moves into replaceable `/workspace/.riftcli/` intelligence |
