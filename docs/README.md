...
| Repository Consistency Observer | **N1.8.0 PATCH 10.35 / TORTURE GATE PENDING** | [`systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`](systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md) | Patch 10.34 live-proved producer-bound PI cache v4 reload across true process death, then exposed restart-order dependency divergence on an unchanged clean repo; Patch 10.35 canonicalizes PI graph ordering and makes ambiguous dependency resolution fail closed; Builder/install/restart parity + remaining adversarial matrix required; staged subsystem planner remains blocked |
