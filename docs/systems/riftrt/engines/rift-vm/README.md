...
`rift-exec-v1` is a bootstrap executable ABI, not a claim that Rift++ Core V1 is already complete. Installed `0.6.0-bootstrap` lowers its proven Gates 0–5 subset into this format, including Gate 5 state/effect support after installed cross-launch persistence evidence passed. Local `0.7.0-bootstrap` stages Gate 6A finite numeric/parameter identity on the same ABI and must still pass CI/build plus installed proof. If later Core semantics need a stronger typed/optimized ABI, that evolution happens as a named compatible executable version rather than silently changing `rift-exec-v1`.
