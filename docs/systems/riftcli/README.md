...
The hard N2-before-N3 barrier is satisfied: N2.12 is promoted, the full Batch V2 restoration chain through first-class MCP Batch is promoted, and the N3 machine-authority prelude is live on run 389. N3.0 is promoted on installed source `001354552af5a0a8034af82a4341187a8abeda52`, Builder run `36210504706` / run number `390`. One genuine native gap was then proven before N3-M1 activation: `/workspace/.riftcli/` could not reach the frozen candidate-impact/propagation authorities because the native driver correctly forbids `rift_workspace_exec`. Source now adds hidden read-only `rift_cli_project_intelligence` → `workspace.projectIntelligenceReadOnly`, restricted to canonical `candidateImpact()` or bounded `projectPropagation()`. It is absent from public MCP `tools()`, forbidden in Batch, and never aliases to workspace-exec. The bridge is now Builder/install/live-proven on source `f439e9bb6d61d41997ec92636b6e4e0ec89c5912`, Builder run `36216594210` / run number `391`. Canonical candidate-impact and bounded propagation both completed through the native driver with no authorization bypass, and the public MCP catalog remained exactly 24 tools. N3-M1 (N3.1+N3.2) is promoted from live `/workspace/.riftcli/` evidence and N3-M2 (N3.3+N3.4) is ready. N3-M2 work pauses only until one shared Builder/install proves the exact-cap persisted-job admission repair and the hidden five-kind read-only N3 evidence bridge (`candidate-impact|propagation|contracts|claims|proofs`). Canonical N2 memory runtime authority remains inactive.
