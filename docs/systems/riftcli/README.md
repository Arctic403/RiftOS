...
The current host boundary is now fixed: MCP/RiftShell ingress enters the existing RiftOS Local Agent, the Local Agent invokes RiftCLI as an internal intelligence/tool layer, and the existing CLI execution supervisor delegates only through bounded RiftOS authorities. The post-run397 convergence checkpoint makes this mandatory for ordinary public authority-bearing MCP calls: they enter Local Agent → RiftCLI before execution. Passive `rift_info`/`rift_debug`, first-class Batch control, and direct `rift-cli help|status|architecture|enable|disable` trust-kernel controls remain intentional non-authority/bootstrap exceptions. The `rift-cli` shell command is retained as a compatibility/development surface, and `riftos-agent cli ...` exposes the same bounded path specifically for Local Agent hosting tests. Both route through `RiftOsLocalAgent` before reaching the native CLI core; raw `riftos-agent intelligence ...` remains unexposed.
...
- public authority-bearing MCP calls keep the outer bounded MCP deadline while using a shorter bounded CLI-route deadline internally; passive status/debug and Batch control remain separate bounded exceptions.
...
The hard N2-before-N3 barrier is satisfied: N2.12 is promoted, the full Batch V2 restoration chain through first-class MCP Batch is promoted, and the N3 machine-authority prelude is live on run 389. N3.0 is promoted on installed source `001354552af5a0a8034af82a4341187a8abeda52`, Builder run `36210504706` / run number `390`. One genuine native gap was then proven before N3-M1 activation: `/workspace/.riftcli/` could not reach the frozen candidate-impact/propagation authorities because the native driver correctly forbids `rift_workspace_exec`. The hidden read-only `rift_cli_project_intelligence` → `workspace.projectIntelligenceReadOnly` bridge was promoted on source `f439e9bb6d61d41997ec92636b6e4e0ec89c5912`, Builder run `36216594210` / run number `391`; it remains absent from public MCP `tools()`, forbidden in Batch, and never aliases to workspace-exec. Installed source `f559194b107ea2a0d3787c45684ee41b8856633c`, Builder run `36220917679` / run number `393`, then live-proved the exact-cap persisted-job admission repair and the full frozen five-kind bridge `candidate-impact|propagation|contracts|claims|proofs`: a new Batch job was admitted at the 32-record bound, the store remained at 32 with zero nonterminal jobs, and direct hidden Contracts/Claims/Proofs requests all completed with per-operation authorization and no bypass while the public MCP catalog remained exactly 24 tools. N3-M1 (N3.1+N3.2) remains promoted. N3-M2 (N3.3+N3.4) is implementation-ready but activation is blocked until the universal public MCP → Local Agent → RiftCLI authority convergence is Builder/install/live-proven. Replaceable `/workspace/.riftcli/` package 0.2.1 additionally reconnects passive DebugHub reads through `rift-cli debug` → native driver → `rift_debug`, with no direct QuickJS/debug mutation authority. Installed run397 confirms source/build/runtime continuity for the queued/manual-approved Observer pre-push gate and AWOL fence; the first real manual-approval push remains the live proof. N3-M2 resumes only after that protection and the universal CLI authority-convergence checkpoint are live-proven together. Canonical N2 memory runtime authority remains inactive.
