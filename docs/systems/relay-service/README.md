...
SSE output uses JSON-RPC notifications `notifications/riftcli/event` and SSE `id` equal to the event sequence. A slow SSE writer is removed when backpressure indicates it is not keeping up instead of accumulating an unbounded queue. Heartbeats now also track whether the stream queue makes drain progress: two consecutive heartbeat observations with queued data and no forward drain progress evict the subscriber as backpressure. This closes the case where a browser/network stack keeps an HTTP stream object alive after the visible tab is closed but never propagates request abort/cancel to the Durable Object.

Live N1.7 stress on 2026-09-21 opened eight external SSE browser streams successfully and verified the ninth was rejected with `Too many MCP SSE subscribers`. Closing all tabs did not immediately propagate disconnect for every stream; deliberate ~37 KiB CLI event pressure then evicted six of the stale streams via `backpressureDropped`, confirming the bounded queue path works live. The no-drain heartbeat rule is the source fix for the remaining idle-stale case and still requires redeploy/live proof.

The relay requests replay from the device rather than persisting event payloads. Accepted device events receive `cli.ack`, and the Android client tracks the highest ACKed sequence in memory **and** persists the same monotonic scalar in app-private relay preferences. On process startup, `RiftMcpRelayClient` restores that value before the first authenticated `device.hello` and sends it as `cliAckSequence`; the relay monotonically restores `lastCliSequence` from it before calculating `relay.ready.cliResumeAfter`. This closes both ordinary socket/DO-state-loss recovery and the Android process-death case where the previous RAM-only ACK cursor would otherwise reset to zero. Active subscriber cursors can still lower the resume point when older events are genuinely required. No Durable Object storage or event-payload persistence is added.
