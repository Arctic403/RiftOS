...
Live N1.7 stress on 2026-09-21 opened eight external SSE browser streams successfully and verified the ninth was rejected with `Too many MCP SSE subscribers`. Closing all tabs did not immediately propagate disconnect for every stream; deliberate ~37 KiB CLI event pressure then evicted six stale streams through `backpressureDropped`, proving the queue bound works. Two idle entries remained, motivating the stable-session/direct-stream/lease repair above. That repair still requires redeploy/live proof.
