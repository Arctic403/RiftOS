...
- N1.8.0 adds `test-rift-repository-consistency-v1.mjs`, which independently checks stable fact/edge identities, separate content hashes, traversal-order-independent graph hashing, bounded/verified private cache wiring, the existing `project kind=consistency` surface and the prohibition on a second observer-owned source index. Promotion additionally requires an installed adversarial torture suite covering cold/warm determinism, content-only edits, add/delete/rename/move/copy, cache corruption/recovery, restart, concurrency, mutation-during-scan races, exact bounds, path/identity cases, exclusions/oversized inputs, dependency graph shapes, resource pressure, result-surface parity and clean-rebuild differentials. Any defect becomes permanent regression coverage before promotion; the future staged subsystem planner remains implementation-blocked until this gate passes;
