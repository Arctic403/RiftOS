...
- `scripts/test-riftllm-bridge.mjs` — fixed standalone RiftLLM Binder API identity/allowlist, Keystore pairing, guarded Workspace publication, bounded A/B Text Encoding Lab method surface and shell-boundary contract.
- `scripts/test-riftllm-text-encoding-bridge.mjs` — executable fixed A/B Text Encoding Lab transfer contract: 192 KiB chunking, exact offsets/hashes, fixed artifact/held-out sources, candidate identity and background-job start/status semantics.
