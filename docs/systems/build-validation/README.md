...
- `scripts/test-rift-memory-n2-final-v1.mjs` gates the separate N2.12 final correctness/adversarial source state: exact frozen contract-file and internal identity hashes, all 12 correctness-corpus IDs, all eight zero-tolerance thresholds, M1-M6 runtime evidence aggregation, BuildConfig source/run continuity, dual ARM ABI build wiring, ARM32 installed-device vs ARM64 build-parity distinction, exactly three read-only `rift_info` lanes, inactive Local Agent/RiftCLI authority, and an explicit ban on timing/comparative benchmark execution; N2.12 carries null promotion source/run until Builder/install/live evidence passes;
