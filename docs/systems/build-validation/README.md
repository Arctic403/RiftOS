...
The external Builder also requires package.json to keep the wiring, transport, docs, RiftCLI push, Batch V2 and DebugHub entrypoints reachable from npm run check. Post-N2 Batch hardening additionally requires `RiftCliPersistentJobStore.kt` and `RiftCliRecoveryPolicy.kt` in the mandatory Android source snapshot and `scripts/test-rift-cli-batch-v2.mjs` to lock sealed bounded persistence, exact plan hashing, step-progress/execution-CWD journaling, non-bypass lease metadata, restart `recovery_required` semantics, blind/whole-job replay prohibition, system-owned retry/idempotency policy with no caller override, recovery plan revalidation/hash checks, enabled-gate recovery authority, opt-in `rollbackPolicy=on-failure`, system-owned rollback-capability prevalidation, 128 KiB per-entry / 512 KiB total private rollback snapshots, public snapshot-byte redaction, post-state drift verification, reverse-order rollback, explicit rollback terminal/cancellation states, recovered rollback journal validation, restart-aware job controls, the bounded Local Agent Batch translator contract, the exact 24-tool MCP catalog, five bounded first-class `rift_batch_*` controls, their ToolHost → Local Agent routing, the max-16 public submit schema, exact-cap persisted-job admission that trims only the oldest terminal record when a truly new job arrives at the 32-job cap while preserving 24-hour normal retention and all nonterminal/recovery-required jobs, and the requirement that internal `rift_cli_batch`/`rift_cli_job_*` controls remain non-public.
...
- N2.0-N2.2 are promoted; N2-M1 promotion is proven on installed source `694c1e31a6c3f4bd4317edd121208be894be2586`, Builder run `36048901054` / run number `346`, with real process-restart `processRestartRecovered=true`, `crashRollbackRecovered=true` and `restartPromotionReady=true`; the immutable N2.0 freeze contract remains byte-frozen and canonical memory runtime authority remains inactive;
...
- `riftmemory/n2-phase-authority.json` preserves N2.0-N2.11 promoted with N2.1 + N2.2 carrying run-346 promotion evidence, N2.3 + N2.4 carrying installed source `18f1156075e08cb94573a9392031ac64552313f2` / run-350 evidence, N2.5 + N2.6 carrying installed source `62382a94f50dd6052e1754c1496da2a0f794c0af` / run-355 evidence, N2.7 + N2.8 carrying installed source `d39960832a701311461058670b5b93597ae612c9` / run-368 evidence, N2.9 carrying installed source `d650e57dff09a878f02edfef7e175ed02d42f750` / run-370 evidence, and N2.10 + N2.11 carrying installed source `921d32ff295921be8783ca4ce8395ba5ee029553` / run-376 evidence; N2-M1 + N2-M2 + N2-M3 + N2-M4 + N2-M5 + N2-M6 are promoted, N2.12 is promoted on installed source `9e75b0f76fd61ba80ca4c241a41532253bdb4c47` / run `378` and N2 is complete, and the six-macro N2.1-N2.11 execution plan preserves separate per-phase authority;
- `scripts/test-rift-memory-n2-m1-v1.mjs` independently gates promoted N2.1 model/ledger semantics and N2.2 SQLite transaction/content/integrity/restart semantics, including structured v0→v1 migration, copied-DB tamper detection, an armed uncommitted process-death transaction, and next-process `restartPromotionReady`; the four M1 Kotlin files remain mandatory Android source-snapshot entries;
- `scripts/test-rift-memory-n2-m2-v1.mjs` independently gates N2.3 reconciliation outcomes/authority/conflict/supersession/invalidation/project+branch isolation and N2.4 bounded temporal/current/graph rebuild/point-in-time semantics; all three M2 Kotlin files are mandatory Android source-snapshot entries and the M2 diagnostic must appear in exactly three `rift_info` lanes while Local Agent/RiftCLI canonical memory authority remains inactive;
- `scripts/test-rift-memory-n2-m3-v1.mjs` independently gates N2.5 bounded reversible consolidation/semantic derivation and N2.6 belief/prediction/reflection/difference semantics, including project/source bounds, provisional derived-trust ceiling, explicit discrepancy objects, scoring, non-destructive stronger-observation resolution and close/reopen durability; all three M3 Kotlin files are mandatory Android source-snapshot entries and the M3 diagnostic must appear in exactly three `rift_info` lanes while canonical runtime authority remains inactive;
- `scripts/test-rift-memory-n2-m4-v1.mjs` independently gates N2.7 SKILL/FAILURE/CAUSAL/COMMITMENT/POLICY specialist semantics and N2.8 NO/FAST/DEEP/FORENSIC exact/entity/project/temporal/graph/BM25/vector retrieval, evidence-first arbitration, project isolation, bounded payload projection (field/array/depth/node/scalar/char caps) and fail-closed context budgets; all three M4 Kotlin files are mandatory Android source-snapshot entries, the M4 diagnostic must appear in exactly three `rift_info` lanes, and Local Agent/RiftCLI must not reference the M4 engines/self-test;
- `scripts/test-rift-memory-n2-m5-v1.mjs` independently gates N2.9 bounded Observer/Validator evidence snapshots, incomplete-evidence fail-closed behavior, stale/replay blocking, cross-project isolation, failed-validation reconciliation, false-completion reopening through canonical COMMITMENT reconciliation, persisted discrepancy evidence, close/reopen durability, diagnostic-only authority and exactly three `rift_info` attachment lanes; both M5 Kotlin files are mandatory Android source-snapshot entries and Local Agent/RiftCLI must not reference the M5 adapter/self-test;
- `scripts/test-rift-memory-n2-m6-v1.mjs` independently gates promoted N2.10 independent RiftStore/SQLite interface conformance and promoted N2.11 integrity/poisoning/crash/restart/resource hardening, including mandatory M6 source ownership, AtomicFile whole-state sealing and pre-read bounds, exact three-lane `rift_info` diagnostics, dual `arm64-v8a`/`armeabi-v7a` build targets, inactive runtime authority, SQLite-reference preservation and explicit benchmark deferral; promotion is pinned to installed source `921d32ff295921be8783ca4ce8395ba5ee029553` / run `376`, with identical cross-backend fixture SHA-256 and live crash/cold-restart recovery;
- `scripts/test-rift-memory-n2-final-v1.mjs` gates the promoted N2.12 final correctness/adversarial state: exact frozen contract-file and internal identity hashes, all 12 correctness-corpus IDs, all eight zero-tolerance thresholds, M1-M6 runtime evidence aggregation, BuildConfig source/run continuity, dual ARM ABI build wiring, ARM32 installed-device vs ARM64 build-parity distinction, exactly three read-only `rift_info` lanes, inactive Local Agent/RiftCLI authority, and an explicit ban on timing/comparative benchmark execution; N2.12 promotion is pinned to installed source `9e75b0f76fd61ba80ca4c241a41532253bdb4c47` / run `378`, with 12/12 corpus scenarios passing, all eight zero-tolerance outcomes at zero, every weakest-link category green, source/build/runtime continuity true, ARM32 installed proof true and ARM64 build/correctness parity true without claiming ARM64 installed execution;
- the retained `src/riftmemory-control.js` controller remains unpackaged/non-authoritative;
- Project Intelligence classifies only the exact versioned Observer/N2/N3 machine-authority JSON paths as build-config repository state, preventing machine authority edits from becoming unclassified proof changes without broadly treating JSON as config; the N3 path prelude is installed on source `3e9cfb5b7514e3a82d4d739fa0f3d92aba1ba23b`, Builder run `36209390484` / run number `389`;
- `scripts/test-rift-cli-n3-contract-v1.mjs` gates the promoted N3.0 baseline and the one-time N3-M1 evidence bridge: exact frozen contract bytes, run-389 prelude continuity, installed run-390 N3.0 promotion identity, analysis-only/no-planner/no-mutation/no-memory-activation authority, exact PI-v2 substrate bounds, eleven adversarial correctness classes, zero-tolerance impact failures, N3-M1/M2/M3 compaction, separate N3.6 final gate, hidden `rift_cli_project_intelligence` routing to `workspace.projectIntelligenceReadOnly`, public MCP absence, Batch prohibition, the exact five-kind allowlist `candidate-impact|propagation|contracts|claims|proofs`, canonical `candidateImpact()`/bounded `projectPropagation()` reuse, canonical `projectOverview(... contracts|claims|proofs ...)` reuse, and an explicit ban on aliasing the bridge to `workspace.exec`;
- the N2.12-before-N3 barrier, N3.0 promotion gate and hidden PI bridge gate are satisfied. The bridge is live on source `f439e9bb6d61d41997ec92636b6e4e0ec89c5912`, Builder run `36216594210` / run number `391`; live candidate-impact returned `rift-semantic-impact-v1`, live propagation returned complete `rift-semantic-propagation-v1`, bypass flags stayed false, and the public MCP manifest remained exactly 24 tools. N3-M1 local implementation is active in replaceable `/workspace/.riftcli/` intelligence.
...
- N1.8.1 adds `test-rift-integrity-v1.mjs`, which locks the separate read-only integrity lane above the promoted N1.8.0 graph. Historical promotion proof remains analyzer v6 / PI cache v9 / structural-v4 on installed source `198a3f31e22a5d385378fee087aa5f115aed6d5a`. Current source advances to analyzer v7 / PI cache v10 / `bounded-structural-v5-conservative` after the N2-M3 pre-build nullable-flow audit proved run 353 could return clean/zero-unresolved evidence for unsafe nullable MemoryStore dereferences. The permanent gate now also requires bounded typed-nullable tracking, `kotlin-nullable-dereference`, recognized explicit non-null proofs, executable-code masking and cache invalidation so pre-hardening rows cannot be reused. Live v7 fixture proof remains required on the next installed build;
...
- N1.8.5 adds `test-rift-proof-obligations-v1.mjs`, which locks the read-only `project kind=proofs` planner over exact candidate-impact evidence. Strong affected-test selection is limited to changed tests, direct dependency/dependent evidence and changed-symbol references; path-affinity tests remain supplemental and cannot satisfy the affected-test obligation. Missing strong evidence must become an unresolved obligation, while incomplete impact, API/build/deletion/multi-project/unclassified risk escalates to deep verification. The planner is evidence-only (`executesVerification=false`), general suites cannot substitute for affected tests, result sets are bounded, and deterministic `proofsSha256` binds only the canonical `project-local-plan-v1` plan; workspace-global candidate IDs, semantic-impact hashes and changed-symbol lists are diagnostics and cannot perturb an unrelated project's proof identity. The regression also locks exact-at-limit/+1 behavior against the Kotlin cap expressions for changes, selected tests, supplemental tests and preview rows. `MAX_CHECKS=128` and `MAX_OBLIGATIONS=1024` are defensive ceilings above the current structural maximum (4 check requests / 12 obligation call sites), so their pre-add guards are exact/+1 tested and the call-graph cardinality itself is locked. Post-promotion FAIL-005 hardening additionally requires bounded literal config-read direct-dependent discovery for changed machine-authority files plus explicit `authority-consumer-tests` proof obligations: direct consumers are required evidence, while a missing consumer set is unresolved/fail-closed. The semantic-impact regression locks contract + M1 + M2 + M3 + M4 plus `validate-rift-docs.mjs` as current `n2-phase-authority.json` direct verification consumers; live proof remains pending the next install. FAIL-006 additionally gates maintained regression discovery sanity in `test-rift-documentation-claims-v1.mjs`: `readdirSync('scripts')` test filters must compile and match at least one real sibling maintained test, `MAX_REGRESSION_DISCOVERY_PATTERNS=512` is enforced, zero-match/invalid discovery is blocking, and the corrected semantic-impact test-file filter is `/^test-.*\.(?:mjs|js)$/`; FAIL-007 additionally broadens `test-rift-documentation-claims-v1.mjs` coverage so regression-literal ownership includes maintained `*Read(...)` aliases and direct/prefixed `*Assert.ok(alias.includes('literal'))` assertions under executable-code masking; direct and marker-loop assertions share the existing 2,048 fail-closed bound and `regression-literal-marker-missing`. The N2 contract now checks `fun isMachineAuthorityPath(` plus the exact machine-authority path literals instead of the removed local variable text. Run `359` passed the FAIL-005/006/007 source gates and live-proved authority-consumer selection plus zero-match discovery detection; its live claims pass exposed an escaped-JavaScript-literal false positive in the new direct-literal parser. Current source uses shared `decodeRegressionLiteral(...)` normalization for direct + loop literals, and the independent JS regression now scans `*Read/*Assert` direct literals itself with `decodeRegressionMarker(...)`, making RiftGit's escaped `$input` assertion a permanent false-positive control. Run `360` / source `aa03a7a4a1229ff942a0b35ec086a9a9c81c35bf` passed the decoder rebuild and final live claims proof: baseline claims were clean, the RiftGit escaped-literal control stayed clean, a representative direct wrong-literal fixture produced `regression-literal-marker-missing`, and exact fixture removal restored the identical clean claims hash. FAIL-005/006/007 post-promotion hardening is closed. The N1.8.5 hardening pass additionally makes `test-rift-workspace-records.mjs` lock the 256-record rolling history, 60-second Workspace Records budget, tracking-policy-v2 migration, separately persisted bounded candidate-session evidence, watcher-aware currentness/shared ignore policy, and stable candidate-state identity that excludes record-chain/history/pruning/session provenance; `test-rift-semantic-impact-v1.mjs` locks the zero-change no-index fast path, bounded candidate-project-scoped PI refresh, forbids whole-workspace candidate-impact refresh, and requires the full evidence manifest SHA to be attached only after semantic hashing; transport regressions lock the 75/90/100/110/120-second request chain plus 300-second SSE lease;
- N1.8.6 adds `test-rift-observer-adversarial-v1.mjs` to the mandatory source gate. The canonical seven-fixture corpus is SHA-256 bound as `da0fd2e1313a9c00fc3e0e382abf3e32cc8d33503e922e11365041953ec55012`, requires explicit fail/hold outcomes across syntax/graph/claims/historical layers, includes four true-positive isolation cases and three false-positive controls, verifies deterministic replay, and chains the promoted consistency/integrity/propagation/contracts/claims/proofs regressions so layer hardening cannot silently disappear. This is correctness validation only; performance/comparative benchmarking remains deferred until the full CLI is complete/live;
