...
- `scripts/test-rift-cli-n3-contract-v1.mjs` gates N3.0 before engine implementation: exact frozen contract bytes, run-389 prelude continuity, promoted N1.8/N2/Batch prerequisites, analysis-only/no-planner/no-mutation/no-memory-activation authority, exact PI-v2 substrate bounds, eleven adversarial correctness classes, zero-tolerance impact failures, N3-M1/M2/M3 compaction and a separate N3.6 final gate;
- the N2.12-before-N3 barrier is satisfied; ROADMAP.md now requires N3.0 contract/baseline promotion before N3.1-N3.6 implementation may proceed.
