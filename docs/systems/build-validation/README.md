...
Because the Gradle list is now exact 49/49, the Builder consumes the same mandatory native snapshot rather than maintaining another stale source list.
...
- Patch 9 added `test-rift-verification-planner-v1.mjs`, which locks candidate-derived test/security/dependency planning, deterministic required check ids, exact target coverage, stale-plan binding and zero MCP/trust expansion;
