...
- `scripts/test-rift-vm.mjs` — executable `rift-exec-v1`/`riftvm-1` validation and execution contract, including checked arithmetic, nominal struct/enum operations, composite host-boundary denial, declared imports, hard step/stack/call limits, fake-prevalidation resistance, no eval/process path and the `.rift` + `main.rxe` package fixture.
