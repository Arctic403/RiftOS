...
The Activity receives a permitted root/entry, normalizes requested path segments and serves matching local files through WebView request interception (`responseFor`). The preview WebView can navigate within the served local content and supports back navigation; it is destroyed with the Activity. Renderer loss is recorded through `RiftRendererCrashGuard`, the dead preview WebView is removed/destroyed, and only the preview Activity closes.
...
- Preview renderer crash terminates RiftOS -> `onRenderProcessGone`/shared crash-guard handling regressed.
...
Test HTML with relative CSS/images, plain previewable content, nested paths, encoded/traversal attempts, missing file, back navigation, renderer process loss and Activity destruction. Renderer loss must close only preview and leave the RiftOS process alive.
