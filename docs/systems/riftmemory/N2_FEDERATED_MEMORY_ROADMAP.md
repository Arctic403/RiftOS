...
**Current source status:** N2.0-N2.2 are promoted and **N2-M1 is PROMOTED** on installed source `694c1e31a6c3f4bd4317edd121208be894be2586`, Builder run `36048901054` / run number `346`, with real restart/crash rollback proof and clean Observer continuity. **N2-M2 is SOURCE-IMPLEMENTED / PROMOTION PENDING:** N2.3 owns bounded candidate validation, trust/authority reconciliation, explicit COMMITTED/PROVISIONAL/QUARANTINED/REJECTED outcomes, conflict search, supersession/invalidation and dirty projection semantics; N2.4 owns bounded current/temporal/graph projections, entity/dependency/provenance/supersession/contradiction/invalidation edges, bi-temporal point-in-time reconstruction, and exact disposable projection rebuild from canonical ledgers. `RiftMemoryN2M2SelfTest` is diagnostic-only/read-only through `rift_info`; canonical Local Agent/RiftCLI memory authority remains inactive. Builder/install runtime proof is required before N2.3 or N2.4 promotion.
