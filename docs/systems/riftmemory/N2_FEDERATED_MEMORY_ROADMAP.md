...
**Current source status:** N2.0-N2.12 are promoted; N2 is complete. **N2-M1 is PROMOTED** on installed source `694c1e31a6c3f4bd4317edd121208be894be2586`, Builder run `36048901054` / run number `346`, with real restart/crash rollback proof and clean Observer continuity. **N2-M2 is PROMOTED** on installed source `18f1156075e08cb94573a9392031ac64552313f2`, Builder run `36067080197` / run number `350`: every N2.3/N2.4 self-test field was true, SQLite integrity was clean, and post-install Observer continuity returned zero findings with proofs `mode=none`. N2.3 owns bounded candidate validation, trust/authority reconciliation, explicit COMMITTED/PROVISIONAL/QUARANTINED/REJECTED outcomes, conflict search, supersession/invalidation and dirty projection semantics; N2.4 owns bounded current/temporal/graph projections, entity/dependency/provenance/supersession/contradiction/invalidation edges, bi-temporal point-in-time reconstruction, and exact disposable projection rebuild from canonical ledgers. Canonical Local Agent/RiftCLI memory authority remains inactive. **N2-M3 is PROMOTED** on installed source `62382a94f50dd6052e1754c1496da2a0f794c0af`, Builder run `36075992479` / run number `355`: N2.5 adds bounded reversible observation→episode→episode-cluster→pattern→semantic-candidate derivation with canonical evidence/version lineage and a hard provisional derived-trust ceiling; N2.6 adds explicit belief/prediction/reflection/discrepancy separation, expected-vs-observed comparison, deterministic bounded surprise/importance scoring, unresolved discrepancy state and stronger-observation reconciliation without destructive overwrite. `RiftMemoryN2M3SelfTest` is diagnostic-only/read-only through `rift_info`; all N2.5/N2.6 diagnostics passed on run 355 with clean SQLite integrity, live nullable-flow v7 detection, live regression-literal detection, exact 2,048 assertion completeness and +1 fail-closed behavior. `RiftMemoryN2M2SelfTest` remains diagnostic-only/read-only through `rift_info`; canonical Local Agent/RiftCLI memory authority remains inactive. **N2-M4 is PROMOTED** on installed source `d39960832a701311461058670b5b93597ae612c9`, Builder run `36094853587` / run number `368`. N2.7 adds `RiftMemoryProceduralFailureV1.kt`: bounded SKILL/FAILURE/CAUSAL/COMMITMENT/POLICY specialist lanes over existing canonical records and reconciliation, explicit provisional derived-trust ceilings, failure-recurrence detection, evidence-backed causal confidence, restart-visible open commitments, and protected-policy authority that learned memory cannot grant itself. N2.8 adds `RiftMemoryRetrievalContextV1.kt`: NO/FAST/DEEP/FORENSIC routing, bounded exact/entity/project/temporal/graph/BM25/vector disposable indexes, bounded payload projection with field/array/depth/node/scalar/char caps, evidence-first arbitration where similarity cannot outrank required direct verified evidence, strict project isolation, and a fail-closed token-bounded Context Compiler. `RiftMemoryN2M4SelfTest.kt` proves N2.7 and N2.8 separately in `app-private/riftmemory-diagnostics/n2-m4-proof.sqlite` and is wired read-only through exactly three `rift_info` lanes with `runtimeAuthority=false`. `riftmemory/n2-phase-authority.json` separately promotes N2.7 and N2.8 on `d39960832a701311461058670b5b93597ae612c9` / run `368` and marks N2-M4 promoted while canonical runtime remains inactive. Installed M4 diagnostics and SQLite integrity passed, M1-M3 continuity stayed green, and the final harmless phase-authority fixture selected contract + M1 + M2 + M3 + M4 + `validate-rift-docs.mjs` with zero unresolved obligations before exact restoration returned all Observer views clean. No comparative/performance benchmark is part of M4. **N2-M5 is PROMOTED** on installed source `d650e57dff09a878f02edfef7e175ed02d42f750`, Builder run `36100246139` / run number `370`: `RiftMemoryObserverValidatorLoopV1.kt` turns bounded Observer/Validator snapshots into evidence-only canonical candidates, requires complete/fresh/project-matching evidence before target-state reconciliation, blocks stale/replayed snapshots, and never directly mutates trusted truth. `RiftMemoryN2M5SelfTest.kt` proves stale-state correction, failed-validation correction, false-completion reopening, incomplete-evidence fail-closed behavior, project isolation, bounds, persisted difference evidence and restart durability in `app-private/riftmemory-diagnostics/n2-m5-proof.sqlite`; ToolHost exposes it read-only through exactly three `rift_info` lanes with `runtimeAuthority=false`. All N2.9 diagnostics passed, SQLite integrity is clean, M1-M4 continuity stayed green, and the clean post-install Observer baseline returned zero findings with proofs `mode=none`. **N2-M6 is PROMOTED** on installed source `921d32ff295921be8783ca4ce8395ba5ee029553`, Builder run `36110733372` / run number `376`: N2.10 adds independent bounded `RiftStoreMemoryStoreV1.kt` conformance behind the exact frozen `MemoryStore` interface while SQLite remains the N2 production/reference backend; N2.11 adds sealed fail-closed RiftStore state, bounded poisoning/capacity guards, crash/cold-restart probes, and aggregates promoted SQLite crash/corruption plus exact projection-rebuild evidence. `RiftMemoryN2M6SelfTest.kt` remains diagnostic-only with `runtimeAuthority=false`; run 376 produced identical SQLite/RiftStore fixture SHA-256 `76b163bf330e40ef9c2b0e6a4683817f5accea48827cc2b1f6cca78b9536f969`, all ordinary N2.11 hardening checks passed, and a real force-stop/reopen process boundary proved `riftStoreCrashRollbackRecovered=true`, `coldRestartRecovered=true`, and `restartPromotionReady=true` while M1-M5 stayed green. No comparative/performance benchmark is part of M6.
...
**Current promoted implementation:** `RiftMemoryObserverValidatorLoopV1.kt` is a backend-neutral evidence adapter over `RiftMemoryCandidateV1` + `RiftMemoryReconciliationV1`; complete Observer/Validator snapshots may reconcile target state, incomplete snapshots remain provisional evidence only, source sequence/snapshot replay is rejected, cross-project scope is rejected, and Validator-backed commitment correction can reopen false completion through normal canonical reconciliation. `RiftMemoryN2M5SelfTest.kt` proves the closed loop in a separate diagnostic SQLite DB with `runtimeAuthority=false`; N2.9 promotion is pinned to installed source `d650e57dff09a878f02edfef7e175ed02d42f750`, Builder run `36100246139` / run number `370`, with every N2.9 diagnostic true and clean post-install Observer continuity.
...
**Current source implementation:** `RiftStoreMemoryStoreV1.kt` is an independent `AtomicFile` backend implementing the exact frozen `RiftMemoryStoreV1` surface, with staged atomic transactions, content-addressed blobs, record-version/current state, evidence/event hash chains, snapshots, dirty projections, a canonical whole-state SHA-256 seal and a 16 MiB pre-read store bound. `RiftMemoryN2M6SelfTest.kt` runs the same deterministic logical fixture through SQLite and RiftStore and requires exact observable conformance; SQLite remains the reference/production backend, `productionReplacement=false`, and comparative performance remains deferred.
...
**Current promoted implementation:** the M6 diagnostic adds RiftStore fsck/integrity, state-seal tamper detection, invalid/missing evidence rejection, oversized-state rejection before read/allocation, bounded blob reads, cold-restart persistence, and a deliberately uncommitted process-death rollback probe. It reuses the already-promoted M1 SQLite crash/corruption proof and M2 exact projection-rebuild proof. Gradle still builds both `arm64-v8a` and `armeabi-v7a`; current source records ABI facts and bounded ABI-neutral semantics but does not claim ARM64 installed-device execution from the present 32-bit device. On installed source `921d32ff295921be8783ca4ce8395ba5ee029553`, Builder run `36110733372` / run number `376`, `restartPromotionReady=true`; the real force-stop/reopen process boundary also proved `riftStoreCrashRollbackRecovered=true` and `coldRestartRecovered=true`.
...
**Current source implementation:** `RiftMemoryN2FinalSelfTest.kt` is a diagnostic-only final evidence aggregator over M1-M6. It re-runs the promoted diagnostics and emits one frozen 12-scenario correctness table, all eight zero-tolerance outcomes, weakest-link categories, immutable contract/corpus/store/threshold identities, SQLite reference integrity, RiftStore conformance/restart state, installed BuildConfig source/run continuity, ARM32 installed-device evidence and ARM64 build/correctness parity without claiming ARM64 installed-device execution. `scripts/test-rift-memory-n2-final-v1.mjs` hashes the actual frozen contract and binds those runtime constants to the exact contract identities, scenario IDs, threshold keys, dual-ABI Gradle wiring and exactly three read-only `rift_info` lanes. The diagnostic remains `runtimeAuthority=false`; N2.12 is promoted on installed source `9e75b0f76fd61ba80ca4c241a41532253bdb4c47`, Builder run `36147252584` / run number `378`, after all 12 frozen scenarios passed, all eight zero-tolerance outcomes remained zero, every weakest-link category passed, source/build/runtime continuity passed, ARM32 installed-device proof passed, and ARM64 correctness/build parity passed without claiming ARM64 installed-device execution. No comparative/performance benchmark is executed or scored.
