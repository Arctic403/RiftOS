...
**Current source implementation:** the M6 diagnostic adds RiftStore fsck/integrity, state-seal tamper detection, invalid/missing evidence rejection, oversized-state rejection before read/allocation, bounded blob reads, cold-restart persistence, and a deliberately uncommitted process-death rollback probe. It reuses the already-promoted M1 SQLite crash/corruption proof and M2 exact projection-rebuild proof. Gradle still builds both `arm64-v8a` and `armeabi-v7a`; current source records ABI facts and bounded ABI-neutral semantics but does not claim ARM64 installed-device execution from the present 32-bit device. `restartPromotionReady` is intentionally separate from ordinary diagnostic `ok` and must become true after a real process restart before M6 promotion.
