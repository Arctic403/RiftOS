...
- Live `vortex` / `vortex-agent` / `riftos-agent` / `riftllm-agent` operations, experimental `rift-cli`, `chat` bundle creation and the `rift` local-platform family are explicitly listed as non-reversible and rejected by atomic batch preflight; repo/vault/build/memory operations use their own transaction/durability semantics and cannot truthfully participate in the batch engine's ordinary RiftFS rollback contract.
