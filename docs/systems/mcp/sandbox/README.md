...
N1.8.5 adds the read-only `project kind=proofs` planner implemented by `RiftProofObligationsV1`. It consumes exact candidate-impact evidence plus the existing validation command surface and emits a deterministic verification plan without executing checks. Directly affected tests require strong evidence (changed-test, direct dependency/dependent, or changed-symbol reference); path-affinity tests are supplemental only and cannot satisfy the affected-test obligation. Missing strong test evidence becomes an explicit unresolved obligation, while API/build/deletion/multi-project/incomplete/unclassified risk deterministically escalates the plan to deep verification. General `npm` suites are classified as supplemental/general checks and cannot substitute for an affected test. `proofsSha256` binds the canonical plan, bounded previews cap returned rows, and the planner exposes `executesVerification=false`. N1.8.5 is promoted on installed source `08146ba30ef713ac895e390adf5d515ef78fa728`, Builder run `36014191151` / run number `335`, after live project-isolation, affected-test, deterministic create/delete, restart-parity and N1.8.0-N1.8.4 continuity proof.
