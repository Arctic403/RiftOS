...
- N1.8 Repository Consistency Observer content-truth + cache-provenance hardening above PI-v2/Workspace Records: installed torture proved the original content-only regressions fixed on `7ee74c5034bb14c30945d28971c56f35424e5301`, and installed Patch 10.34 source `1c0e644f7424d30f20acdc1da5cf540833802ff2` live-proved producer-bound PI cache v4 rejection/rebuild and trusted reload across true Android process death; that restart exposed an unchanged-repository graph divergence caused by traversal-order dependency fallbacks, so Patch 10.35 now canonicalizes graph iteration and resolves ambiguous Python/symbol candidates only when unique; Builder/install/restart parity plus the remaining torture matrix are still required before promotion, while the staged subsystem/domain planner stays design-only;
...
The locked N1.8 Repository Consistency Observer reuses Workspace Records + Project Intelligence V2 rather than creating a second index. N1.8.0 must survive the installed full-repository torture gate before promotion; if it fails, the foundation is hardened and re-tested rather than stacking another observer layer on top. Patch 10.34 is installed and proved PI cache v4 producer provenance across a true Android force-stop/restart (`cacheLoadStatus=loaded`, trusted analyzer/build producer), but that same restart exposed a new hard blocker: the unchanged clean repository produced a different canonical consistency graph because PI-v2 dependency resolution still contained traversal-order `firstOrNull` fallbacks. Patch 10.35 is source-implemented to canonicalize graph iteration and fail closed on ambiguous Python/symbol dependency candidates; local Android validation is green, but Builder/install/restart torture proof is still required. The future everyday execution model is design-locked as staged subsystem/domain scanning with graph-driven frontier expansion, while clean full-repository scans remain the independent correctness oracle. That staged planner is not implemented until N1.8.0 promotion. The canonical architecture, torture matrix, benchmark corpus and N1.8.0-N1.8.7 gates are in `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`. N2 is blocked until N1.8 promotion.
