...
- **N1.8 Repository Consistency Observer — HARD PRE-N2 GATE; N1.8.0 source-implemented, promotion pending.** The N1.8.0 foundation now exists as `RiftRepositoryConsistencyObserver.kt` above Workspace Records + Project Intelligence V2. It derives a bounded fact/edge graph from the existing PI-v2 graph instead of rescanning source, gives facts/edges/findings stable content-independent IDs plus separate content hashes, canonicalizes/sorts the graph into a deterministic SHA-256 identity, persists only a verified rebuildable app-private cache, and exposes the foundation through the existing `project kind=consistency` view. `test-rift-repository-consistency-v1.mjs` is wired into the Builder check chain and independently locks ordering/identity/hash/no-second-index invariants. Local Android source validation is green. The first installed whole-repo proof exposed that the observer was consuming PI-v2's 120-file/600-edge presentation caps and therefore returned `complete=false`. Source now uses the same PI-v2 graph builder with observer-only 1024-file/1024-edge internal bounds; the current repo is 258 files / 758 dependencies and fits safely within the observer's 4096-fact/4096-edge ceilings. **Promotion now requires the full installed torture matrix, not merely one `complete=true` scan:** cold/warm determinism, repeated runs, content-only edits, add/delete/rename/move/copy, cache corruption/recovery, process death/restart, concurrent readers, mutation-during-scan races, exact bound tests, path/identity stress, exclusion/oversize handling, dependency-shape stress, resource pressure, result-surface parity and clean-oracle differential checks. Any false-clean, silent skip/truncation, nondeterministic graph, stale-cache acceptance, stale node, mixed-state race, hang/crash or authority mutation blocks promotion and becomes a permanent regression case. **A staged subsystem scan planner is architecture-locked but implementation-blocked until N1.8.0 passes that torture gate.** After promotion, normal operation should scan the owning subsystem/domain first and expand only across affected graph edges/proof obligations, while periodic/uncertain-state full-repo clean scans remain the correctness oracle. N1.8.1-N1.8.7 remain pending and cover syntax/import integrity, semantic propagation, cross-boundary contracts, documentation claims, focused verification, adversarial benchmarks/ablations, and installed-device promotion. Canonical specification: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
