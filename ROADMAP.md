...
- **N1.5 Persistent push/events — installed/source-complete; final external receipt proof pending.** Reuse the persistent device WSS for sequenced RiftCLI lifecycle events, bounded device-owned replay, per-subscriber WebSocket/SSE cursors, ACKs, reconnect recovery and inline small terminal results. Current source adds passive DebugHub tracing for event creation, WSS queueing, replay and Cloudflare ACK receipt; that instrumentation needs the next APK build/install before live use. Rapid polling is no longer the steady state; poll/list/cancel remain fallback controls. No Durable Object event-payload writes.
