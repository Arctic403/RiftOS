...
- **N1.5 Persistent push/events — source-complete; promotion pending.** Reuse the persistent device WSS for sequenced RiftCLI lifecycle events, bounded device-owned replay, per-subscriber WebSocket/SSE cursors, ACKs, reconnect recovery and inline small terminal results. Rapid polling is no longer the steady state; poll/list/cancel remain fallback controls. No Durable Object event-payload writes.
- **N1.6 RiftCLI Batch V2 — source-complete; promotion pending.** New `rift_cli_batch` lane with at most 16 fully prevalidated sequential steps, one global authority reservation for the whole plan, bounded per-step results, stop/continue failure policy, cancellation checks, per-step push events and `rift-cli-batch` provenance. This does **not** resurrect retired RiftShell `batch` or multi-op `rift_workspace_exec`.
- **N1.7 abuse/reconnect/batch stress — next promotion gate.** Prove reconnect replay/ACK, duplicate filtering, slow-subscriber behavior, large-result fallback, cancellation between batch steps, no interleave, both failure policies and cleanup on the installed build.
