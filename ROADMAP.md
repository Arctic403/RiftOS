...
- **Batch V2 restoration/hardening — ACTIVE POST-N2 / B1+B2A+B2B + LOCAL AGENT BATCH + FIRST-CLASS MCP BATCH PROMOTED.** Batch V2 may be exposed back to external reasoning only through **external reasoning → MCP/relay → RiftOS Local Agent → RiftCLI `rift_cli_batch` → bounded RiftOS authorities**. The model submits one bounded batch job; Local Agent/RiftCLI own full-plan prevalidation, sequential execution, one global authority reservation, cancellation/recovery, stop/continue policy, bounded per-step results, per-step push events and `rift-cli-batch` provenance. Initial external cap remains **16 steps**. Transport loss must not cause ambiguous re-execution: job identity/replay protection and device-owned job state remain authoritative, and reconnect/recovery must resume observation rather than resubmit mutations. Observer/Validator/ownership/security gates may not be bypassed by batch. **Model-facing `rift_workspace_exec` remains exactly one operation per call, retired RiftShell `batch` remains disabled, and neither retired path is ever the mechanism used to restore batching.** Re-exposure occurs only after single-command persistence, process/disconnect recovery, cancellation, CLI-local batch recovery and failure/rollback semantics are live-proven together; then Local Agent exposure precedes MCP exposure, and only after that does N3 begin.
...
- **N2 Federated Rift Memory Kernel — N2.0-N2.12 PROMOTED / N2-M1 + N2-M2 + N2-M3 + N2-M4 + N2-M5 + N2-M6 PROMOTED; N2 COMPLETE; CANONICAL MEMORY RUNTIME INACTIVE.** N2-M1 (N2.1 + N2.2) is promoted on installed source `694c1e31a6c3f4bd4317edd121208be894be2586`, Builder run `36048901054` / run number `346`, after real process restart proved `processRestartRecovered=true`, `crashRollbackRecovered=true` and `restartPromotionReady=true`; post-restart claims/consistency/integrity/contracts were clean and proofs returned `mode=none` with exact baseline `a4f92d9d654407528cbf7a48c30d5f12b4aa5ab503d2903ce8952d1ecbf0aecc`. N2-M2 is promoted on installed source `18f1156075e08cb94573a9392031ac64552313f2`, Builder run `36067080197` / run number `350`: `RiftMemoryReconciliationV1.kt` owns candidate validation/reconciliation, explicit COMMITTED/PROVISIONAL/QUARANTINED/REJECTED outcomes, authority/conflict/supersession/invalidation semantics and projection invalidation; `RiftMemoryTemporalGraphV1.kt` owns bounded rebuildable current/temporal/graph projections and point-in-time reconstruction; `RiftMemoryN2M2SelfTest.kt` live-proved all N2.3/N2.4 diagnostics with `ok=true`, `runtimeAuthority=false`, clean SQLite integrity, and post-install Observer continuity returned zero findings with proofs `mode=none`. N2-M3 is promoted on installed source `62382a94f50dd6052e1754c1496da2a0f794c0af`, Builder run `36075992479` / run number `355`: `RiftMemoryConsolidationV1.kt` owns bounded reversible observation→episode→cluster→pattern→semantic candidate derivation with preserved evidence/version lineage and a provisional derived-trust ceiling; `RiftMemoryBeliefDifferenceV1.kt` owns explicit belief/prediction/reflection/discrepancy separation, deterministic surprise/importance scoring and stronger-observation discrepancy resolution through normal reconciliation; `RiftMemoryN2M3SelfTest.kt` is diagnostic-only/read-only through `rift_info`. N2.1-N2.11 execute as six macro patches: M1=N2.1+N2.2, M2=N2.3+N2.4, M3=N2.5+N2.6, M4=N2.7+N2.8, M5=N2.9, M6=N2.10+N2.11. Macro patches are execution groupings only; every contained subphase keeps separate evidence/promotion authority, and N2.12 remains a separate final correctness/adversarial gate. Build one canonical evidence/event/transaction/reconciliation authority with multiple rebuildable specialist cognitive engines rather than one monolithic algorithm or competing truths. N2 starts with a replaceable SQLite reference `MemoryStore`; RiftStore remains an experimental backend, but any comparative replacement decision is deferred until the full RiftCLI stack is 100% complete and live. N2.0-N2.12 cover the canonical JSON model, immutable/content-addressed evidence, bi-temporal history, trust/reconciliation, temporal graph, episodic/consolidation/semantic/belief/skill/failure/causal/commitment/predictive lanes, router/fusion/arbitration/Context Compiler, Observer+Validator closed loop, and poisoning/fsck/crash/rebuild/scale hardening. During N2 implementation, promotion evidence is correctness/durability/resource-safety only; public/private comparative benchmarks, performance comparisons and ablations are deferred post-CLI. Build/runtime failures are fixed immediately and their Observer prevention lessons are accumulated in `docs/PAST_FAILS.md`; after N2.12/full RiftCLI completion, all applicable ledger lessons must be hardened into Observer/Validator coverage and live-proven clean before any benchmark begins. Full specification: `docs/systems/riftmemory/N2_FEDERATED_MEMORY_ROADMAP.md`.
- **N3 Architecture/impact engine — N3.0 PROMOTED; hidden PI bridge PROMOTED on run `391`; N3-M1 (N3.1+N3.2) PROMOTED; N3-M2 (N3.3+N3.4) BLOCKED ON CLI AUTHORITY CONVERGENCE; run 393 exact-cap Batch admission repair + five-kind N3 evidence bridge LIVE-PROVEN; universal public MCP → Local Agent → RiftCLI authority convergence SOURCE IMPLEMENTED / BUILDER + INSTALL + LIVE PROOF PENDING.** The bridge is live-proven for canonical `rift-semantic-impact-v1` candidate impact and bounded `rift-semantic-propagation-v1`, remains absent from the public 24-tool MCP catalog, is forbidden in Batch, and never aliases to `rift_workspace_exec`. N3.1+ now executes in replaceable `/workspace/.riftcli/` intelligence unless a new native primitive is genuinely proven missing. Before N3-M2 resumes, ordinary public authority-bearing MCP calls must route through Local Agent → RiftCLI; passive `rift_info`/`rift_debug`, Batch control, and the direct `rift-cli` trust-kernel controls are the only intentional public exceptions. `rift_workspace_exec` remains one operation per public call but is now a CLI tool-lane and Batch-capable authority. N3 remains analysis-only, canonical N2 memory runtime authority remains inactive, and N4 Planner remains blocked until N3.6 promotion. Full specification: `docs/systems/riftcli/N3_ARCHITECTURE_IMPACT_ROADMAP.md`.
