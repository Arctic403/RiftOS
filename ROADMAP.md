...
- **N3 Architecture/impact engine — N3.0 PROMOTED on installed source `001354552af5a0a8034af82a4341187a8abeda52`, Builder run `36210504706` / run number `390`; N3-M1 INTERNAL PI BRIDGE SOURCE IMPLEMENTED / BUILDER + INSTALL PROOF PENDING.** N1.8, N2.0-N2.12, the full Batch V2 restoration chain, N3 machine-authority prelude and N3.0 contract/baseline gate are satisfied. N3 remains analysis-only: it cannot mutate, authorize, grant capabilities or activate the canonical N2 memory runtime. One missing native primitive was proven: `/workspace/.riftcli/` could not reach the frozen candidate-impact/propagation authorities because the driver correctly forbids `rift_workspace_exec`. Source now adds one hidden read-only `rift_cli_project_intelligence` bridge to the existing canonical PI methods; it is absent from the public MCP catalog, forbidden in Batch, and never aliases to workspace-exec. N3-M1 local intelligence activates immediately after this bridge is built/installed. N4 Planner remains blocked until N3.6 promotion. Full specification: `docs/systems/riftcli/N3_ARCHITECTURE_IMPACT_ROADMAP.md`.
