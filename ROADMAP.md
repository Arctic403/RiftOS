...
- **N1.7H Local Agent hosting boundary — REQUIRED BEFORE FURTHER CLI INTELLIGENCE.** RiftCLI is not a sibling transport or standalone controller. MCP/RiftShell ingress enters the existing RiftOS Local Agent, and the Local Agent hosts RiftCLI as its internal gated intelligence/tool layer before bounded RiftOS authorities execute. The native process-local `CONFIRM-EXPERIMENTAL` switch remains the only enable gate, defaults OFF on every process start and is non-persistent. The existing driver protocol remains only as an internal compatibility/execution mechanism behind the Local Agent host. **Once this host boundary is validated, all further CLI intelligence expansion (Observer integration, Memory, Planner, Command Registry, Execution Supervisor, Debug/history expansion, Security/validation expansion) is frozen until N1.8.0 is promoted.**
- **N1.8 Repository Consistency Observer — HARD PRE-N2 GATE; Patch 10.36 PI-cache integrity hardening is source-implemented, runtime proof pending.** Patch 10.35 installed source `8ecc5433dcca153a669b09f942bc91e64965c0fa` live-proved exact true-restart parity at graph `d9064a2ba8f1742116aa0f88e67110a5022907a9bf699bab9d61f2a27e7109d7` with 1017 facts / 1071 edges / 258 files, ten identical warm runs, concurrent/reentrant consistency reads, unique-match-or-unresolved ambiguity handling, package-qualified Kotlin resolution, coherent mutation-during-scan behavior, exact 1024-file and 1024-dependency boundaries, 2 MiB exact/above semantic-file behavior, path/content identity, metadata-only evidence and 32-project observer-cache eviction/rebuild. Continued torture audit then found a new hard blocker in PI cache v4: producer provenance and file SHA were verified, but the persisted semantic payload (`symbols`/`dependencies`) itself had no integrity seal, so a syntactically valid corrupted row with the correct file SHA could be reused and potentially create false-clean semantics. Patch 10.36 upgrades PI persistence to schema v5 with a canonical SHA-256 whole-payload seal; missing or mismatched seals fail closed before producer-bound semantic rows load. Current source additionally hardens long stable-key identity (full normalized key for IDs; bounded 2048-character stored/display key with SHA-256 suffix), makes the optional observer snapshot fail-soft on >4 MiB/write/verification failures, and uses overflow-safe exact byte-budget checks. Installed torture proof now confirms the 128 MiB semantic budget accepts exact and fails closed at +1, while the 256 MiB repository-content hash budget has no hash-bound reason at exact and fails closed at +1. Local Android source validation is green; Builder/Kotlin/JS regression compile, install, v4→v5 rejection/rebuild, true restart v5 reload/canonical graph parity, long-key runtime proof, and fail-soft oversized-observer-cache runtime proof are still required before N1.8.0 promotion. **The staged subsystem/domain scan planner remains architecture-locked but implementation-blocked until this hardened N1.8.0 passes the complete torture gate.** N1.8.1-N1.8.7 remain pending. Canonical specification: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
