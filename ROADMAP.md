...
- **N1.8 Repository Consistency Observer — HARD PRE-N2 GATE; content-truth + semantic-cache provenance hardening source-implemented, promotion/torture restart pending.** Installed source `5de7f5065160b2bbe263e8cc0d179f6099af4347` proved repaired whole-repo coverage (`complete=true`, 1017 facts / 1072 edges) and ten identical warm graph hashes, then exposed content-only false-clean behavior. Patch 10.33 corrected that with exact repository-file SHA/size evidence and content-bound graph facts. Installed source `7ee74c5034bb14c30945d28971c56f35424e5301` re-ran the three original kill cases successfully: same-size source edit, README content-only edit, and delete/recreate of the same path with unrelated bytes all changed canonical repository content state correctly while preserving stable path identity where intended. The second-round torture review then found another promotion blocker: persisted PI-v2 semantic entries were SHA-bound to repository bytes but not to the analyzer/build that produced their symbols/dependencies, so an app update could reuse stale semantics for unchanged files. Source now uses PI cache schema v4 with exact file evidence **plus producer provenance** (`RiftSourceIntelligenceV2.VERSION` + trusted full `BuildConfig.RIFT_SOURCE_SHA`); missing/mismatched/untrusted producer caches are rejected, local/unknown builds do not receive trusted persistent semantic reuse, and diagnostics expose cache load/rejection state. Repository Fact Graph/cache remains v2 and content-bound. Local Android source validation is green; external Builder/Kotlin compile and a fresh install must prove old-cache rejection/rebuild followed by current-producer cache load after restart before the remaining torture matrix continues. **The staged subsystem/domain scan planner remains architecture-locked but implementation-blocked until this hardened N1.8.0 passes the complete torture gate.** N1.8.1-N1.8.7 remain pending. Canonical specification: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
