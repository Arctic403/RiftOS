...
- **N1.8 Repository Consistency Observer — HARD PRE-N2 GATE; Patch 10.35 semantic-order determinism hardening source-implemented, runtime proof pending.** Installed source `5de7f5065160b2bbe263e8cc0d179f6099af4347` proved repaired whole-repo coverage (`complete=true`, 1017 facts / 1072 edges) and ten identical warm graph hashes, then exposed content-only false-clean behavior. Patch 10.33 corrected that with exact repository-file SHA/size evidence and content-bound graph facts. Installed source `7ee74c5034bb14c30945d28971c56f35424e5301` re-ran the three original kill cases successfully. Patch 10.34 then bound PI cache v4 semantics to exact file SHA plus trusted analyzer/build producer provenance. Installed source `1c0e644f7424d30f20acdc1da5cf540833802ff2` live-proved old-cache rejection and, after a true Android force-stop/reopen, successful v4 reload with `cacheLoadStatus=loaded`, analyzer version 2, exact trusted source SHA and no rejection reason. That restart immediately exposed another hard promotion blocker: the unchanged clean RiftOS repository retained 1017 facts / 1072 edges / 258 files but canonical graph SHA changed from `93bc7228ea90278bbb1d2421bad89850c1422c590bdd0113698f4f24de06736b` to `e3868cc6f0a260036a7383d5a78fa26eedb805d97e3fdb191f0e9d3856241dbd`. Audit traced this to traversal-order-sensitive `firstOrNull` dependency fallbacks in PI-v2 combined with cache reload ordering. Patch 10.35 now canonicalizes project graph iteration, adds unambiguous qualified Kotlin/Java import matching, and changes Python/symbol fallback resolution to unique-match-or-unresolved so ambiguity cannot become a false edge. Local Android validation is green; Builder/Kotlin compile, install, true restart parity and the remaining torture matrix are still required. **The staged subsystem/domain scan planner remains architecture-locked but implementation-blocked until this hardened N1.8.0 passes the complete torture gate.** N1.8.1-N1.8.7 remain pending. Canonical specification: `docs/systems/workspace/REPOSITORY_CONSISTENCY_OBSERVER.md`.
