...
function normalize(path=""){
  ...
  if(value===".rift"||value.startsWith(".rift/"))throw new Error("Workspace metadata is not exposed to the live HTML surface");
  return value;
}
...
async function invoke(method,args={}){
  switch(method){
    case "info":return {...await workspace.info(),surface:"sandboxed-local-html",watch:watcherState};
    ...
    case "copy":return workspace.copy(normalize(args.path),normalize(args.newPath),{overwrite:false});
...
function mount(container){
  ...
  container.innerHTML='<iframe class="rift-workspace-live-frame" title="Rift Workspace Live" src="./workspace-live/index.html" sandbox="allow-scripts"></iframe>';
...
console.info("[RiftWorkspaceLive] sandboxed local HTML workspace surface ready");
