...
export const RIFTPP_CORE_VERSION='0.10.0-bootstrap';
...
const MAX_SOURCE_CODE_UNITS=4*1024*1024;
const MAX_STRING_BUILDER_UNITS=4*1024*1024;
...
const SUPPORTED_PRIMITIVES=new Set(['unit','bool','u8','u32','s32','f64','string']);
const BUILTIN_VALUE_TYPES=new Set(['SourceText','TextCursor']);
...
const BUILTIN_GENERIC_TYPES=new Set(['Vec','Buffer','Slice','StringBuilder','Option','Result']);
const COMPARABLE_PRIMITIVES=new Set(['unit','bool','u8','u32','s32','f64','string']);
const ORDERED_PRIMITIVES=new Set(['u8','u32','s32','f64','string']);
...
const TEXT_BUILTINS=new Set(['source_text','string_builder']);
const NUMERIC_TEXT_BUILTINS=new Set(['parse_u32','parse_s32','parse_f64','format_u32','format_s32','format_f64']);
const BYTE_BUILTINS=new Set(['u8_to_u32','u8_from_u32']);
const PRELUDE_NAMES=new Set(['print','value_sha256',...CHECKPOINT_BUILTINS,...STRING_BUILTINS,...TEXT_BUILTINS,...NUMERIC_TEXT_BUILTINS,...BYTE_BUILTINS,...REPAIR_BUILTINS,...SOFTWARE_BUILTINS]);
  ...
  typeRef(){
    ...
    try{
      ...
      if(this.consume('<')){
        ...
        else if(name==='StringBuilder'){capacityRaw=this.expectKind('number','StringBuilder byte capacity','E0118').value;this.expect('>');}
...
function parseInt(raw,node,type){const clean=raw.replaceAll('_','');let value;try{value=BigInt(clean);}catch{semanticFail('E0202',`invalid integer literal '${raw}'`,node,'numeric semantics');}const bounds=type==='u8'?[0n,255n]:type==='s32'?[-2147483648n,2147483647n]:[0n,4294967295n];if(value<bounds[0]||value>bounds[1])semanticFail('E0203',`${type} literal is out of range`,node,'checked integer semantics');return value.toString();}
...
function linkRiftPlusPlusCoreProgramV1(rootSource,moduleSources={}){
  ...
  const rewriteType=(rec,type)=>{if(!type)return null;const args=(type.args||[]).map(item=>rewriteType(rec,item));const name=['Vec','Buffer','Slice','StringBuilder','Option','Result'].includes(type.name)?type.name:resolveType(rec,type.name,type);return Object.freeze({...type,name,args:Object.freeze(args)});};
...
class Codegen{
  ...
  internGeneric(kind,args,capacity=null){const key=(kind==='Vec'||kind==='Buffer')?`${kind}<${args[0]},${capacity}>`:kind==='StringBuilder'?`StringBuilder<${capacity}>`:`${kind}<${args.join(',')}>`;if(!this.genericTypes.has(key))this.genericTypes.set(key,Object.freeze({kind,args:Object.freeze([...args]),capacity}));return key;}
  ...
  typeExists(name){return SUPPORTED_PRIMITIVES.has(name)||BUILTIN_VALUE_TYPES.has(name)||this.structs.has(name)||this.enums.has(name)||this.genericTypes.has(name);}
  stateDescriptor(type,node,seen=new Set(),depth=0){
    ...
    if(generic?.kind==='StringBuilder')semanticFail('E0379',`checkpoint type '${type}' is unsupported; StringBuilder is compiler/runtime working text`,node,'Gate 1B text foundation');
    if(type==='SourceText'||type==='TextCursor')semanticFail('E0378',`checkpoint type '${type}' is unsupported; text working values are not persistent state`,node,'Gate 1B text foundation');
  ...
  ensureType(typeRef){
    ...
    if(typeRef.name==='StringBuilder'){if(typeRef.args?.length||typeRef.capacityRaw==null)semanticFail('E0370','StringBuilder requires StringBuilder<N>',typeRef,'Gate 1B text foundation');const raw=String(typeRef.capacityRaw).replaceAll('_','');if(!/^[1-9][0-9]*$/.test(raw))semanticFail('E0371','StringBuilder capacity must be a positive UTF-16 code-unit count',typeRef,'Gate 1B text foundation');const capacity=Number(raw);if(!Number.isInteger(capacity)||capacity<1||capacity>MAX_STRING_BUILDER_UNITS)semanticFail('E0372',`StringBuilder capacity must be 1..${MAX_STRING_BUILDER_UNITS}`,typeRef,'Gate 1B text foundation');return this.internGeneric('StringBuilder',[],capacity);}
    ...
    if(!this.typeExists(typeRef.name))semanticFail('E0200',`type '${typeRef.name}' is not implemented or declared in this Core module`,typeRef,'bootstrap type support',`Primitive types supported now: ${[...SUPPORTED_PRIMITIVES].join(', ')}; SourceText/TextCursor and local struct/enum plus Vec/Buffer/Slice/StringBuilder/Option/Result types are also supported.`);return typeRef.name;
  }
  collectTypes(){
  ...
  collectSignatures(){
    for(const fn of this.ast.functions){
      if(PRELUDE_NAMES.has(fn.name)||SUPPORTED_PRIMITIVES.has(fn.name)||BUILTIN_VALUE_TYPES.has(fn.name)||BUILTIN_GENERIC_TYPES.has(fn.name)||POISON_NAMES.has(fn.name)||this.topNames.has(fn.name))semanticFail('E0204',`duplicate or reserved top-level name '${fn.name}'`,fn,'name resolution');
  ...
  compileFunction(fn){
    ...
    const compileBuiltinMethod=(expr,expected)=>{
      if(expr.callee.kind!=='Member')return null;const method=expr.callee.member,receiverType=compileExpr(expr.callee.object,null);
      if(receiverType==='SourceText'){
        if(method==='code_unit_len'){if(expr.args.length)semanticFail('E0373','SourceText.code_unit_len expects no arguments',expr,'Gate 1B text foundation');emit({op:'source_code_unit_len'});expectType('u32',expected,expr);return'u32';}
        if(method==='utf8_byte_len'){if(expr.args.length)semanticFail('E0373','SourceText.utf8_byte_len expects no arguments',expr,'Gate 1B text foundation');emit({op:'source_utf8_byte_len'});expectType('u32',expected,expr);return'u32';}
        if(method==='cursor'){if(expr.args.length)semanticFail('E0373','SourceText.cursor expects no arguments',expr,'Gate 1B text foundation');emit({op:'source_cursor'});expectType('TextCursor',expected,expr);return'TextCursor';}
        if(method==='slice'){if(expr.args.length!==2)semanticFail('E0373','SourceText.slice expects start and end UTF-16 code-unit offsets',expr,'Gate 1B text foundation');compileExpr(expr.args[0],'u32');compileExpr(expr.args[1],'u32');emit({op:'source_slice'});const type=this.internGeneric('Result',['SourceText','string']);expectType(type,expected,expr);return type;}
        if(method==='to_string'){if(expr.args.length)semanticFail('E0373','SourceText.to_string expects no arguments',expr,'Gate 1B text foundation');emit({op:'source_to_string'});const type=this.internGeneric('Result',['string','string']);expectType(type,expected,expr);return type;}
        semanticFail('E0373',`SourceText has no method '${method}'`,expr,'Gate 1B text foundation');
      }
      if(receiverType==='TextCursor'){
        const scalar=(op)=>{if(expr.args.length)semanticFail('E0374',`TextCursor.${method} expects no arguments`,expr,'Gate 1B text foundation');emit({op});expectType('u32',expected,expr);return'u32';};
        if(method==='code_unit_offset')return scalar('cursor_code_unit_offset');
        if(method==='line')return scalar('cursor_line');
        if(method==='column')return scalar('cursor_column');
        if(method==='eof'){if(expr.args.length)semanticFail('E0374','TextCursor.eof expects no arguments',expr,'Gate 1B text foundation');emit({op:'cursor_eof'});expectType('bool',expected,expr);return'bool';}
        if(method==='peek_code_unit'){if(expr.args.length!==1)semanticFail('E0374','TextCursor.peek_code_unit expects one u32 code-unit offset',expr,'Gate 1B text foundation');compileExpr(expr.args[0],'u32');emit({op:'cursor_peek_code_unit'});const type=this.internGeneric('Option',['u32']);expectType(type,expected,expr);return type;}
        if(method==='advance'){if(expr.args.length)semanticFail('E0374','TextCursor.advance expects no arguments',expr,'Gate 1B text foundation');emit({op:'cursor_advance'});expectType('TextCursor',expected,expr);return'TextCursor';}
        semanticFail('E0374',`TextCursor has no method '${method}'`,expr,'Gate 1B text foundation');
      }
      const info=this.genericInfo(receiverType);
      if(info?.kind==='StringBuilder'){
        if(method==='len_units'){if(expr.args.length)semanticFail('E0376','StringBuilder.len_units expects no arguments',expr,'Gate 1B text foundation');emit({op:'builder_len'});expectType('u32',expected,expr);return'u32';}
        if(method==='append'){if(expr.args.length!==1)semanticFail('E0376','StringBuilder.append expects one string',expr,'Gate 1B text foundation');compileExpr(expr.args[0],'string');emit({op:'builder_append'});const type=this.internGeneric('Result',[receiverType,'string']);expectType(type,expected,expr);return type;}
        if(method==='append_source'){if(expr.args.length!==1)semanticFail('E0376','StringBuilder.append_source expects one SourceText',expr,'Gate 1B text foundation');compileExpr(expr.args[0],'SourceText');emit({op:'builder_append_source'});const type=this.internGeneric('Result',[receiverType,'string']);expectType(type,expected,expr);return type;}
        if(method==='finish'){if(expr.args.length)semanticFail('E0376','StringBuilder.finish expects no arguments',expr,'Gate 1B text foundation');emit({op:'builder_finish'});expectType('SourceText',expected,expr);return'SourceText';}
        semanticFail('E0376',`StringBuilder has no method '${method}'`,expr,'Gate 1B text foundation');
      }
      if(!info||!['Vec','Buffer','Slice'].includes(info.kind))semanticFail('E0267',`method '${method}' is not defined for ${receiverType}`,expr,'builtin method dispatch');const itemType=info.args[0],kind=info.kind,prefix=kind==='Vec'?'vec':kind==='Buffer'?'buffer':'slice',rule=kind==='Vec'?'bounded collection method':'Gate 1A scalable storage',arityCode=kind==='Vec'?'E0268':kind==='Buffer'?'E0353':'E0361';
    ...
    const compileExpr=(expr,expected=null)=>{
      ...
      try{
      if(expr.kind==='IntLiteral'){const type=expected==='u8'?'u8':expected==='s32'?'s32':expected==='u32'?'u32':'u32';if(expected&&!['u8','u32','s32'].includes(expected))semanticFail('E0201',`type mismatch: expected ${expected}, got integer`,expr,'type checking');emit({op:'const',index:this.constant(type,parseInt(expr.raw,expr,type))});return type;}
      ...
      if(expr.kind==='Unary'){
        ...
        if(expr.op==='+'){const type=compileExpr(expr.expression,expected);if(!['u8','u32','s32','f64'].includes(type))semanticFail('E0209','unary + requires a numeric value',expr,'numeric semantics');return type;}
      ...
      if(expr.kind==='Binary'){
        ...
        if(['<','<=','>','>='].includes(expr.op)){const left=compileExpr(expr.left,null),right=compileExpr(expr.right,left);if(left!==right)semanticFail('E0201',`comparison operands differ: ${left} and ${right}`,expr,'type checking');if(!ORDERED_PRIMITIVES.has(left))semanticFail('E0246',`ordered comparison requires u8, u32, s32, f64 or string; got ${left}`,expr,'comparison semantics');expectType('bool',expected,expr);emit({op:{'<':'lt','<=':'le','>':'gt','>=':'ge'}[expr.op]});return'bool';}
        const preferred=expected&&['u8','u32','s32','f64','string'].includes(expected)?expected:null,left=compileExpr(expr.left,preferred),right=compileExpr(expr.right,left);if(left!==right)semanticFail('E0201',`binary operands differ: ${left} and ${right}`,expr,'type checking');if(expr.op==='+'&&left==='string'){expectType('string',expected,expr);emit({op:'concat'});return'string';}if(!['u8','u32','s32','f64'].includes(left))semanticFail('E0211',`operator '${expr.op}' requires numeric operands`,expr,'numeric semantics');expectType(left,expected,expr);emit({op:{'+':'add','-':'sub','*':'mul','/':'div','%':'mod'}[expr.op]});return left;
      }
      if(expr.kind==='Call'){
        const constructed=enumConstructor(expr.callee,expr.args,expr,expected);if(constructed)return constructed;if(expr.callee.kind==='Member')return compileBuiltinMethod(expr,expected);
        ...
        if(name==='value_sha256'){if(expr.args.length!==1)semanticFail('E0290','value_sha256 expects exactly one bounded value',expr,'Gate 6A parameter identity');const valueType=compileExpr(expr.args[0],null),generic=this.genericInfo(valueType);if(valueType==='SourceText'||valueType==='TextCursor'||generic?.kind==='StringBuilder')semanticFail('E0382',`value_sha256 does not accept compiler-working text type ${valueType}`,expr,'Gate 1B text foundation');emit({op:'value_sha256'});expectType('string',expected,expr);return'string';}
        if(name==='u8_to_u32'){if(expr.args.length!==1)semanticFail('E0383','u8_to_u32 expects one u8 value',expr,'native byte substrate');compileExpr(expr.args[0],'u8');emit({op:'u8_to_u32'});expectType('u32',expected,expr);return'u32';}
        if(name==='u8_from_u32'){if(expr.args.length!==1)semanticFail('E0384','u8_from_u32 expects one u32 value',expr,'native byte substrate');compileExpr(expr.args[0],'u32');emit({op:'u8_from_u32'});const type=this.internGeneric('Result',['u8','string']);expectType(type,expected,expr);return type;}
        if(name==='source_text'){if(expr.args.length!==1)semanticFail('E0375','source_text expects one string',expr,'Gate 1B text foundation');compileExpr(expr.args[0],'string');emit({op:'source_text'});expectType('SourceText',expected,expr);return'SourceText';}
        if(name==='string_builder'){if(expr.args.length!==0)semanticFail('E0375','string_builder expects no arguments',expr,'Gate 1B text foundation');const info=this.genericInfo(expected);if(!info||info.kind!=='StringBuilder')semanticFail('E0375','string_builder requires an expected StringBuilder<N> type',expr,'Gate 1B text foundation');emit({op:'make_string_builder',capacity:info.capacity});return expected;}
        if(name==='parse_u32'||name==='parse_s32'||name==='parse_f64'){if(expr.args.length!==1)semanticFail('E0380',`${name} expects one string`,expr,'Gate 1B numeric text');compileExpr(expr.args[0],'string');const valueType=name==='parse_u32'?'u32':name==='parse_s32'?'s32':'f64';emit({op:name});const type=this.internGeneric('Result',[valueType,'string']);expectType(type,expected,expr);return type;}
        if(name==='format_u32'||name==='format_s32'||name==='format_f64'){if(expr.args.length!==1)semanticFail('E0381',`${name} expects one numeric value`,expr,'Gate 1B numeric text');const valueType=name==='format_u32'?'u32':name==='format_s32'?'s32':'f64';compileExpr(expr.args[0],valueType);emit({op:name});expectType('string',expected,expr);return'string';}
