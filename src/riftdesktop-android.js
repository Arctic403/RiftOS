...
if(homeButton){homeButton.firstChild.textContent='⊞';const label=homeButton.querySelector('span');if(label)label.textContent='Start';homeButton.title='Start';homeButton.setAttribute('aria-label','Start');}
...
for(const button of dock.querySelectorAll('.dock-btn[data-open]:not([data-open="home"])')){const label=button.querySelector('span')?.textContent?.trim()||button.dataset.open||'App';button.setAttribute('aria-label',label);button.title=label;taskbarScroll.append(button);}
...
function currentGeometry(win){const rect=win.getBoundingClientRect(),stageRect=stage.getBoundingClientRect();return{left:rect.left-stageRect.left,top:rect.top-stageRect.top,width:rect.width,height:rect.height};}
...
function saveGeometry(win){if(win.classList.contains('rift-maximized'))return;persistWindowGeometry(win.dataset.app,currentGeometry(win));}
...
function bumpGeometryRevision(win){const next=(Number(win.dataset.riftGeometryRevision)||0)+1;win.dataset.riftGeometryRevision=String(next);return next;}
async function restoreDesktopGeometry(win){const revision=String(win.dataset.riftGeometryRevision||'0'),saved=await readStoredGeometry(win);if(!document.contains(win)||String(win.dataset.riftGeometryRevision||'0')!==revision)return;if(win.classList.contains('rift-maximized'))maximize(win,true);else if(!win.classList.contains('rift-minimized'))applyGeometry(win,saved);}
function windowActionLabel(win,action){const title=win.querySelector('.window-title')?.textContent?.trim()||win.dataset.app||'window';return `${action} ${title}`;}
...
function makeTaskbarWindowButton(id,title,item,pinned=false){
  ...
  icon.textContent=meta.icon;label.textContent=meta.label;button.setAttribute('aria-label',meta.label);button.title=meta.label;button.append(icon,label);return button;
}
...
function syncTaskbar(){
  ...
  dock.querySelectorAll('.dock-btn[data-open]').forEach(btn=>{
    ...
    const id=btn.dataset.open,item=byId.get(id),pinned=taskbarPins.has(id)||btn.dataset.pinned==='true',meta=taskbarMeta(id,item?.title||'');btn.setAttribute('aria-label',meta.label);btn.title=meta.label;
...
async function saveIconGeometry(card){
  try{
    ...
    icons[key]=clampIconGeometry(card,rect.left-bounds.left+workspace.scrollLeft,rect.top-bounds.top+workspace.scrollTop);
    await persistDesktopSettings({icons});
  }catch(_){ }
}
function clampIconGeometry(card,left,top){
  const width=card.offsetWidth||84,height=card.offsetHeight||90;
  return{left:clamp(Number(left)||0,0,Math.max(0,workspace.clientWidth-width)),top:clamp(Number(top)||0,0,Math.max(0,workspace.clientHeight-height))};
}
async function restoreIconGeometry(card){
  try{
    const data=await core?.fs?.readJSON?.('/system/settings/desktop.json',{})||{};
    const saved=data.icons?.[card.dataset.open||card.textContent.trim()];
    if(saved){const geometry=clampIconGeometry(card,saved.left,saved.top);card.style.position='absolute';card.style.left=`${geometry.left}px`;card.style.top=`${geometry.top}px`;}
  }catch(_){ }
}
function clampDesktopIcons(){
  document.querySelectorAll('.app-grid .app-card').forEach(card=>{
    if(card.style.position!=='absolute')return;
    const geometry=clampIconGeometry(card,parseFloat(card.style.left),parseFloat(card.style.top));
    card.style.left=`${geometry.left}px`;card.style.top=`${geometry.top}px`;
  });
}
...
function beginHold(){if(!gesture||gesture.maxPointers!==1||gesture.moved)return;const target=pointerTarget();if(!target)return;const win=target.closest?.('.window.rift-desktop-window');if(win&&!win.classList.contains('rift-maximized')&&target.closest('.window-bar')&&!target.closest('button')){bumpGeometryRevision(win);const rect=win.getBoundingClientRect(),stageRect=stage.getBoundingClientRect();gesture.hold={kind:'window',win,left:rect.left-stageRect.left,top:rect.top-stageRect.top,width:rect.width,height:rect.height};focusVisual(win);return;}if(win&&!win.classList.contains('rift-maximized')&&target.closest('.rift-window-resizer')){bumpGeometryRevision(win);const rect=win.getBoundingClientRect(),stageRect=stage.getBoundingClientRect();gesture.hold={kind:'resize',win,left:rect.left-stageRect.left,top:rect.top-stageRect.top,width:rect.width,height:rect.height};focusVisual(win);return;}gesture.hold={kind:'mouse',target};const common={bubbles:true,cancelable:true,composed:true,clientX:cursorX,clientY:cursorY,button:0,buttons:1};try{target.dispatchEvent(new PointerEvent('pointerdown',{...common,pointerType:'mouse',pointerId:1,isPrimary:true}));}catch(_){}target.dispatchEvent(new MouseEvent('mousedown',common));}
...
window.addEventListener('riftos:launcher-ready',()=>{
  ...
  document.querySelectorAll('.app-card').forEach(card=>{
    ...
    card.addEventListener('pointerdown',event=>{
      ...
      const end=()=>{drag=null;saveIconGeometry(card);card.removeEventListener('pointermove',move);card.removeEventListener('pointerup',end);card.removeEventListener('pointercancel',end);};
