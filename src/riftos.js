...
async function openSettings(){
  ...
  body.innerHTML=`<div class="rift-settings-shell">
    ...
    <section class="rift-settings-page">
      ...
      <div class="rift-settings-group"><div><strong>RiftLLM Dev API</strong><small>Optional local Binder bridge to the standalone RiftLLM APK. Pairing tokens are entered locally and stored only in Android Keystore-backed secret storage.</small></div><div class="rift-settings-actions"><button class="trueos-btn primary" id="settingsRiftLlmPair">Pair / rotate</button><button class="trueos-btn" id="settingsRiftLlmStatus">Status</button><button class="trueos-btn" id="settingsRiftLlmUnpair">Unpair</button></div></div>
  ...
  body.querySelector("#settingsRiftLlmPair").onclick=async()=>{try{if(!window.RiftLlmBridge)throw new Error("RiftLLM bridge is not loaded");out.textContent=JSON.stringify(await window.RiftLlmBridge.pair(),null,2);}catch(error){out.textContent=error.message;}};
  body.querySelector("#settingsRiftLlmStatus").onclick=async()=>{try{if(!window.RiftLlmBridge)throw new Error("RiftLLM bridge is not loaded");out.textContent=JSON.stringify(await window.RiftLlmBridge.status(),null,2);}catch(error){out.textContent=error.message;}};
  body.querySelector("#settingsRiftLlmUnpair").onclick=async()=>{try{if(!window.RiftLlmBridge)throw new Error("RiftLLM bridge is not loaded");out.textContent=JSON.stringify(await window.RiftLlmBridge.unpair(),null,2);}catch(error){out.textContent=error.message;}};
...
async function runShell(raw,print,state,context={}){
  const disabledBatchMatch=String(raw||"").trim().match(/^batch(?:\s|$)/i);
  if(disabledBatchMatch)throw new Error("DISABLED: RiftShell batch commands are disabled because they can hang the agent/runtime. Do not use batch or batch --dry-run; use individual commands or MCP file operations instead.");
