...
const STATE_KEY="riftos.browser.state.v5";
const MIGRATION_KEYS=["riftos.browser.state.v4","riftos.browser.state.v3","riftos.browser.state.v2","riftos.browser.state.v1"];
const BOOKMARK_KEY="riftos.browser.bookmarks.v1";
const BACKENDS=["auto","wasm-gecko","native-webkit"];
...
class RiftBrowserKernelService extends EventTarget{
  ...
  async resolveBackend(){
    ...
    if(this.preferredBackend==="wasm-gecko")next=wasmAvailable?"wasm-gecko":"unavailable";
    else if(this.preferredBackend==="native-webkit")next=nativeAvailable?"native-webkit":"unavailable";
  ...
  async setBackend(next="auto"){
    ...
    if(next==="wasm-gecko"){
      ...
      if(this.engineInfo().available!==true)throw new Error("Gecko WASM engine is unavailable or cross-origin isolation is not active");
    }
  ...
  async navigate(input,{tabId=this.activeTabId,replace=false,newTab=false,desktop}={}){
    ...
    if(backend==="wasm-gecko"){
      ...
      result={mode:"unavailable",backend:"unavailable",url:target,reason:this.engineInfo().error||"No RiftBrowser rendering engine is available"};
    }
    ...
    tab.updated=Date.now();
    this.persist();
    const detail={tab:clone(tab),result};
    this.emit("render",detail);
    this.emit("tabs",this.listTabs());
    return detail;
  }
  ...
  bookmark(url=this.activeTab()?.url,title=this.activeTab()?.title){
    ...
    localStorage.setItem(BOOKMARK_KEY,JSON.stringify(rows.slice(0,200)));
    this.emit("bookmarks",rows);
    return rows[0];
  }
  removeBookmark(url){
    const target=normalizeTarget(url),rows=this.bookmarks().filter(row=>row.url!==target);
    localStorage.setItem(BOOKMARK_KEY,JSON.stringify(rows));
    this.emit("bookmarks",rows);
    return rows;
  }
  history(limit=100){
...
console.info("[RiftBrowser] experimental engine browser service online",service.backendStatus());
