...
let inputHandler=()=>{},resizeHandler=()=>{};let seq=0;const pending=new Map(),RPC_TIMEOUT_MS=65000,MAX_PENDING=32;
const rpc=(method,args={})=>new Promise((resolve,reject)=>{if(pending.size>=MAX_PENDING){reject(new Error('RiftRT RPC capacity is full'));return}const id=++seq,timer=setTimeout(()=>{pending.delete(id);reject(new Error('RiftRT RPC timeout: '+method))},RPC_TIMEOUT_MS);pending.set(id,{resolve,reject,timer});postMessage({type:'rpc',id,method,args})});
